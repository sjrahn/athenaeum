"""Describe a classification overlay — the field-spec + tactics lookup for the normalizer.

`corpus overlay <ns>/<id>[/<subtype>]` prints the overlay's `applies_to` (MIME + cues +
excludes), the union of `extended_fields` from the namespace yaml and (when given) the subclass
yaml as a table, the source paths, the description, and the `normalization.guidance`. Use it
before `corpus classify <hash> <ns>/<id> --field …` to know what fields are valid — so the
normalizer never opens overlay yaml directly. Markdown to stdout; non-zero when the overlay
doesn't exist.
"""
...
def run(args: argparse.Namespace) -> int:
    ...
    if schema is None:
        sys.exit(
            f"no schema for namespace {namespace!r} "
            f"(looked for schema/composite/{namespace}/{namespace}.yaml)."
        )
