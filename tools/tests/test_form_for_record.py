...
def test_route_list_matches_any_uri_in_a_list(tmp_path):
    """The block's `uri:` is a list; a rule matches when it hits the SECOND uri, not
    just the first."""
    overlay = """\
applies_to:
  schemes: [routetest]
...
"""
    ...
    assert shape_pkg.form_for_record(post, root) == ("route-host", "special", {"k": "v"})
