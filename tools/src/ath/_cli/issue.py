...
The tracker (forge issues on the repo `tracker.repo` names; `tracker.kind` picks the
dialect — Forgejo or GitHub, spec Part I §2.3) is the source of truth for ticket state.
This command is deliberately READ-ONLY plus one generator:
...
Creating, editing, closing and — above all — COMMENTING are done with the forge's own
CLI (`fj` on Forgejo, `gh` on GitHub), which already does them well. Duplicating them
here would be a second way to do the same thing, and the tracker would start disagreeing
with itself.

Per-record findings file under the Part I §2.3 conventions — `layer:`/`kind:` labels and
git-trailer-style body trailers (`Record:`, `Address:`, `Detector:`, `Ledger:`) — so
`list --label`/`--record` filter the backlog into work packages, and the snapshot groups
its open section by `layer:` label.
...
usage: ath issue <list|show|sync> [options...]
  ...
  list [--state open|closed|all] [--label L] [--record HASH] [--offline]
  show <number>
  sync [--check]

Ticket state lives in the tracker; `sync` mirrors it into the repo so a session can
boot without the network. Use the forge's own CLI (`fj` on Forgejo, `gh` on GitHub)
to record a decision — the trail is the point.
"""
...
def _get(tracker: Tracker, path: str, *, timeout: int = 30):
    ...
    if tracker.kind == "github":
        ...
        # git credential fill answers for github.com through gh's own helper when
        # installed; Basic with a PAT is a supported GitHub API auth route.
...
def fetch_issues(tracker: Tracker, state: str = "all") -> list[dict]:
    ...
    out: list[dict] = []
    page = 1
    page_param = "per_page" if tracker.kind == "github" else "limit"
    while True:
        batch = _get(tracker, f"{tracker.issues_path}?state={state}&{page_param}=50&page={page}")
        if not batch:
            break
...
def _trailer_records(issue: dict) -> list[str]:
    """The blake3 values of the body's `Record:` trailers (spec Part I §2.3), bare —
    a `corpus://` prefix is stripped so a filter argument matches either spelling."""
    out: list[str] = []
    for line in (issue.get("body") or "").splitlines():
        key, _, value = line.partition(":")
        if key.strip().lower() == "record" and value.strip():
            out.append(value.strip().removeprefix("corpus://"))
    return out


def _layer(issue: dict) -> str | None:
    """The issue's `layer:*` label, if it carries one (first alphabetically wins)."""
    for name in _labels(issue):
...
def _render_snapshot(tracker: Tracker, issues: list[dict]) -> str:
    ...
    lines = [
        ...
        f"truth: <{tracker.web}>.",
        "",
        "This file exists so a session can boot without the network, and so the backlog's "
    ...
    def _open_rows(issues_: list[dict]) -> list[str]:
        rows = []
        for i in issues_:
            labs = " ".join(f"`{name}`" for name in _labels(i))
            row = f"- **[#{i['number']}]({i['html_url']})** — {i['title']}"
            rows.append(f"{row}  {labs}" if labs else row)
        return rows

    # Work-package grouping (spec Part I §2.3): when any open issue carries a
    # `layer:*` label, the open section groups by it; ungrouped issues trail.
    layers = sorted({layer for i in open_ if (layer := _layer(i))})
    if layers:
        for layer in layers:
            lines += [f"### {layer}", ""]
            lines += _open_rows([i for i in open_ if _layer(i) == layer])
            lines.append("")
        rest = [i for i in open_ if _layer(i) is None]
        if rest:
            lines += ["### (unlabeled)", ""]
            lines += _open_rows(rest)
            lines.append("")
        lines.pop()  # the trailing group blank; the section separator below adds its own
    else:
        lines += _open_rows(open_)
    lines += ["", "## Closed", ""]
    for i in closed:
...
def run(argv: Sequence[str]) -> int:
    ...
    p_list.add_argument(
        ...
        default=None,
        metavar="HASH",
        help="filter by `Record:` body trailer — a blake3 (or unambiguous hex prefix), "
             "with or without the corpus:// spelling",
    )
    ...
    if args.cmd == "list":
        if args.offline:
            if args.record:
                print("ath issue: --record needs the tracker (bodies aren't in the snapshot)",
                      file=sys.stderr)
                return 2
        ...
        try:
            ...
            return 1
        want_record = (args.record or "").removeprefix("corpus://")
        for i in issues:
            if _is_tombstone(i):
                continue
            labs = _labels(i)
            if args.label and not set(args.label) <= set(labs):
                continue
            if want_record and not any(
                r.startswith(want_record) for r in _trailer_records(i)
            ):
                continue
