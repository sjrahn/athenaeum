...
_COMMANDS: dict[str, tuple[str, str]] = {
    ...
    "mbox-strip":      ("Capture & ingest",       "Strip provider-metadata headers from a mailbox export before ingest (the mail chrome strip)"),
    ...
    "re-stub":         ("Capture & ingest",       "Reset a record to its attested baseline, preserving byte + provenance"),
    "reattest":        ("Capture & ingest",       "Bulk re-derive the attested layer from artifacts (never touches the authored layer)"),
    "redraft":         ("Capture & ingest",       "(retired 3.1) → reattest (attested layer) · queue re-normalize (authored layer)"),
    ...
    "finalize":        ("Normalize",              "Close a claimed pass (gated on authored + formed-where-declared + lint-clean)"),
    ...
    # Query (P1)
    "find":            ("Query",                  "List records matching derived state / mime / origin"),
