...
*(3.0.)* A `stub` has no stored body — its content zone is empty (the body is the `body`
derivation op, §6.2). Decomposing it as-is would hand the normalize pass an empty working dir,
breaking the substrate. So on a stub, decompose **derives the body** (the same op as
`corpus body`) into the working dir, and marks the working dir as derived-at-decompose-time so a
compile from it is an *authoring* act, never mistaken for round-tripping stored bytes. A
`normalized` record (or a 2.x draft with a stored body) decomposes its stored content zone.
"""
...
def run(args: argparse.Namespace) -> int:
    ...
    if status == "stub":
        ...
        # edit; fall back to whatever the stub stores (usually empty) if it can't be derived.
        ...
        try:
            ...
            print(f"  note: body not derivable ({exc}); decomposing the stored (empty) stub.")
