...
| Version | Date | Name | What changed | History |
|---|---|---|---|---|
| 2.0 | — | Interpretive-classification removal | Retires the classify block, the `composite` umbrella, and interpretive `reference` emission — meaning moves to the ledger. | history §12.16 |
| 2.1 | — | Containment amendment | Decouples records from standalone files; a declared member becomes promotable through container streaming. | history §12.17 |
| 3.0 | 2026-07-13 | Derivation revision | Retires the draft stage; a record stores only attested facts and authored form; the TOC becomes a derived rendering over structural segments. | history §12.18 |
| 3.1 | 2026-07-16 | Layers amendment | Retires the stored `status` field; record state (attested / formed / authored) becomes derived; formless becomes the zeroth form. | history §12.19 |
| — | 2026-07-16 | Media-ops increment | Implementation order for the 3.0 track-manifest model: stream attestation, promotion, `form/transcript` + `form/slide-deck`. | history §12.20 |
| 3.2 | 2026-07-16 | Derived-editorial amendment | Title/description become derived from role-marked schema fields, never stored, with an optional deliberate override. | history §12.21 |
| — | 2026-07-16/17 | Form-library growth arc | Five generic form contracts land; the first multi-shape origin adopts route-keyed `form:` declarations. | history §12.22 |
| 3.3 | 2026-07-17 | Terminal-forms amendment | `form/passthrough` and `form/manifest` become declarable terminal contracts; derived state gains `terminal`. | history §12.23 |
| — | — | Stored-address gate | Lint starts enforcing the region grammar on stored addresses; the sweep that caught pixel/corner values passing every prior gate. | history §12.24 |
| — | — | Index-span hole | Names the fabricated `el=1` addresses on artifacts with no addressable element — closed by 3.6, not here. | history §12.25 |
| 3.4 | — | Roster amendment | Per-asset embed blocks collapse into one wholly-attested `members` roster; narration moves off the index entirely. | history §12.26 |
| 3.5 | 2026-06-28 / 2026-07-27 | Faithfulness amendment | States "a record asserts nothing about itself": `description` retires everywhere; a record may carry more than one form section. | history §12.27 |
| 3.6 | 2026-06-25 | Addressing amendment | `el=<path>` replaces the whitelist-indexed `el=<N>`; the HTML address space becomes total and permanent. | history §12.28 |
| 3.7 | — | Derived-envelope amendment | A section's `address` becomes derived from its children, never stored; the whole-record section stops being expressible. | history §12.29 |
| 3.8 | — | Placement amendment | A member's rendering lives only on its own record; a placement displaces the member's content, never the parent's structure. | history §12.30 |
| 3.9 | 2026-07-29 | Region-nesting amendment | A byte belongs to exactly one declared region, the innermost containing it; no record migrates. | history (between §12.30 and §12.31) |
| — | 2026-07-28 | `form/bulletin` retirement | The first form to fail the standing minting test in the present tense; its records re-form onto `form/article`. | history §12.31 |
| — | 2026-07-28 | Mark-in-body | A structural segment's mark text moves from a header scalar into its body, keeping links and position counts honest. | history §12.32 |
| — | 2026-07-29 | The re-segment | Bare-text prose with no element of its own re-segments to its container rather than gaining a fourth address form. | history §12.33 |
| 3.10 | — | Whole-address admissibility | `whole_address` (`admissible` / `forbidden` / `single_unit_only`) bounds when a segment address may be absent. | history §12.34 |
| 3.11 | — | Cut-strategy amendment | A stream leaf's cut boundaries are stamped at promotion as `cutting:`, never silently re-resolved on re-attest. | history §12.35 |
| — | — | Subtitle tracks | Bitmap subtitle cues get the image-member OCR-marker treatment; text-based tracks project faithfully as-is. | history §12.36 |
...
| **25** | 2026-08-18 | **Catalog-metadata amendment** | The presented manifest (§12.1.1) grows from residence map to **light catalog**: identities carry the universal stat facts — size, mtime, ctime, mode, nullable birth time — refreshed by the walk when they drift (free: the walk stats everything anyway), plus an advisory **mime claim** sniffed from the leading bytes once per new identity at hash time, so format-and-placement questions are answerable without touching bytes over the wire. `atime` deliberately excluded; owner ids deferred. Claims stay claims: ingest's detection (§12.3.2) is authoritative and undisturbed; nothing normative may depend on a claim. The location index (§12.9.2) imports `mime_claim` beside the pins; the rest stays queryable in the cached manifest. **Route preference**: a location MAY declare `cost = N` (lower = nearer; undeclared defaults preserve today's class order) — when bytes move, the resolver takes the cheapest residency whose pins verify; locate orders residencies by it; replicate's planner will read it as "copy from the nearest." | — |
| **24** | 2026-08-18 | **Presented-manifest amendment** | An attached location MAY **present its own manifest** (§12.1.1): the **residence scanner** — shared tooling run on the location's host against its *local* filesystem, where inode identity is real — maintains an on-share manifest (working state published as a complete database per generation, unveiled by rename; the byte format is the scanner's own versioned cross-language contract), and attest on a presenting location **reads the manifest instead of walking**, retiring the whole-tree-walk scale limit on remote attached locations. Location-index rows carry their **provenance**: `computed` (attest hashed the bytes itself) or `presented` (a host claim). Trust is the standing rule — the index proposes, the hash disposes: presented rows resolve under the same staleness pins, and every custody-changing act verifies the full blake3 as ever (promotion at mint, adoption during copy). **Residency query**: `corpus locate <blake3>` answers where bytes reside for a bare hash — record or none — across every route; a hash with multiple residencies is the dedup surface. | — |
| **23** | 2026-08-18 | **Claims-and-custody amendment** | Placement gains **origin claims** (§12.1.1): a store location MAY declare `ingest_origins`, a list of origin overlay ids (§7.2) matched against the overlay the minting origin resolved to — the overlay IS the origin's identity, so no second pattern syntax; precedence origin claim → format claim → default → co-located. Migration becomes tooling: **adoption** (`corpus location adopt`) transfers primary custody from an attached location to a store, **non-destructively** — the store gains a blake3-verified copy, the attached original stays in place, its index row shadowed by route order yet surfacable for dedup/reclaim (and countable toward a redundancy floor); and `move` gains a batch form (every standalone copy at a source location, per-record outcomes, continuing past failures). | — |
| **22** | 2026-08-17 | **Placement amendment** | Store locations become first-class custody (§12.1.1): ingest placement lands (format lists, plus `ingest = true` naming one location the DEFAULT destination in place of the co-located tree — most specific claim wins, co-located the fallback), and **move semantics** are defined — `corpus location move` relocates a standalone copy between store locations by copy-verify-then-remove (full blake3 verified before the rename unveils the destination; failed verify leaves the source untouched; attached files never move; the remove half owes redundancy floors `rm`'s honesty). | — |
| **21** | 2026-08-17 | **Custody amendment** | Artifact custody generalizes to **locations** (§12.1.1): a corpus declares any number of byte roots beyond the co-located `artifacts/` tree — **store** locations (content-addressed, corpus-written, local or remote; rclone generalizes the remote transport) and **attached** locations (operator-managed trees whose files stay in place, never renamed, made id-addressable by an attest pass through the derived **location index**, §12.9.2 — the containment model generalized: a location is a container whose manifest is the index). Ingest placement policy picks where new bytes land; resolution falls through routes as ever (§2 unchanged: residence invisible, any route, identical bytes). Mime overlays gain `ref_adapter` (§7.1 — schema-first in the tenancy mold: the reference-dataset resolver derives a dataset's adapter from its mirror record's format; Part I §2.3 demotes manifest `adapter:` to an override) and the sketched `replicas` redundancy floor (max across mime/origin layers; health counts locations; `corpus replicate` planned). gc's cache sweep excludes the persistent derived indexes by name (`hashes.db`, `locations.db`, `refidx/` — §12.8). Retirement begun: Part I's v18 mirror `path:` override deprecates — an attached location over the mirrors directory supersedes it with no bytes moved. | — |
| **20** | 2026-08-16 | **Hash-residency amendment** | Auxiliary hashes unify into one frontmatter field, **`hash:`**, whose value tags carry the classification (§2, §7.6): a bare algorithm id (`sha256:`) is **byte-stable** — never drifts, record-resident, written at ingest or by deliberate flush, byte-verifiable; a `<procedure>@<version>` tag (`html-stampfree@1:`, `eml-stripped@2.1:`) is **procedure-versioned** — index-first, record-resident only by deliberate flush, and deliberately naming the canonicalization rather than the digest algorithm inside it. Similarity-class values (perceptual fingerprints — distance-queryable, not equality-joinable) never reach records at all. First shipped procedure: `html-stampfree@1` **capture-invariant identity** (corpus-injected stamps neutralized — structurally unable to conflate distinct sources; content-canonical stays retired under 3.5's bar, re-enterable only as a new versioned recipe). A **derived hash index** (§12.9.1, `cache/hashes.db`) is the query layer over everything — regenerable, never authoritative; hashing happens only while bytes are in hand — nothing pulls remote bytes just to hash. Recipes resolve as an additive union of three layers (§7.9): a corpus-wide **default set** (`sha256` + `md5` record-resident; `blake3-prefix-ladder` index-only), the mime schema's `derived_hashes` (succeeding `transport_algos` — `text/html` adds `html-stampfree@1`), and the origin overlay's (producer-specific canonicalizations). Sweep: frontmatter `transport:` **renames** to `hash:` on 24,716 records — values byte-identical, frontmatter-only, one commit ( `transport` remains the members-row and `source_transport` vocabulary, where it genuinely names transport identity); the retired `canonical:`/`perceptual:` fields had populations of zero. | — |
...
A record is a single markdown file. The YAML frontmatter at its head carries a small bytes-identity header — what these bytes ARE (the blake3 identity, plus any stored byte-stable hashes — §2's residency rule *(v20)*) and the provenance chain of processing passes. The **record body** below the frontmatter is organized into three **zones**: a **metadata zone** declaring what the artifact is, where it came from, and what members it carries; a **content zone** carrying the rendered content as sections and segments; and an **annotations zone** carrying observations about the record. Each zone holds a small set of HTML-comment block families; §4.3 specifies the grammar.

**The 3.12 freeze (2026-08-07).** 3.12 is the record grammar's declared final shape: the structure below is what records are migrated *to*, and it is stable. Amendments from here are exceptional rather than iterative, and one lands only as a complete change: the normative text, the sweep of every enforcement and guidance site that teaches the retired rule (lint rules, overlay `checks:` and prose, mime-schema guidance, agent briefs — the changelog alone enforces nothing), and the fleet migration story with its measured cost, in the same change. Ruling: sjrahn, 2026-08-07 (#153), on the audit finding that nine amendments in six days had left the spec, the lint gate, and the guidance channel each teaching a different grammar.
...
- *(3.0)* A **multi-track media container** (MP4/ISOBMFF, Matroska, and kin) is a raw archive in the strict sense — a transport whose content is other transports — and is attested the same way: a **track manifest**. Each elementary stream becomes a content-addressed member row addressed `stream_id=<id>` (video stream, audio stream(s), text-based subtitle track(s)), and an attached picture is an ordinary member row; the member's *bytes* are a single-track container of the source's own family rather than a bare elementary stream *(3.12, history §12.20 item 1)* — the address names the track, the pinned form decides what the track's bytes are, and the two are independent; the artifact block carries the container facts (duration, per-stream codecs, dimensions). The content zone carries exactly one thing: the container's **chapter marks** as structural segments (§4.3.2.3) — chapters are byte-marks of the *container* (an MKV `Chapters` element, an MP4 chapter track; both encodings project to the same marks, the mime schema pinning which encoding wins), marking the shared timeline no single track owns. Tracks are **promotable on demand, never exploded**: a promoted track record's bytes materialize through the container (`stream_id=` transform — the `part=<N>` precedent), no bytes copied; extraction semantics are pinned by the mime schema (the mbox `msg=<N>` precedent) so member bytes are deterministic (§11 flags the demuxer-determinism engineering risk). The audio-track record owns the transcript (`?transcribe`, §6.2); the video-track record owns frame work (on-screen text as `text/ocr` is faithful; what a frame *shows* is not recorded at all — the frame marker stands for it, §4.3.2.2); a text-based subtitle track projects faithfully as (time_range, text) segments. Tracks inherit the container's chapter marks at **read time through lineage** (§6.2) — never copied, because the track's own bytes do not carry them (§4.3.2.3, the byte-mark rule). A **single-stream artifact** (a bare MP3, a JPEG) has no member transports to declare and stays a single `work`; a single-*track* media container **is likewise a `work` and does not decompose** *(3.12 — this inverts the original rule, which held it at `disposition: manifest` on the reasoning that the disposition should never flip per record)*. The inversion is forced by what a manifest *is*: a transport whose content is other transports, carrying a roster and no content zone of its own. A single-track container's roster would name itself, and rebuilding it as its own member mints a second identity for identical content — while the `manifest` disposition would strip the content zone that its one track's own work (a transcript, frame markers) has to live in. **The judgment is made from containment lineage, not from a roster-row count**, and that correction was forced by the fleet rather than chosen: the 102 public video containers roster exactly ONE row each — not because they hold one track, but because the second was unpromotable under the superseded pinned form and was declared as a bare track fact instead. A count cannot distinguish *holds one track* from *holds two, one unembeddable*, so it would flip 102 genuine containers to `work` and dissolve the very worklist the transcript migration is tracked by. What separates a leaf from a container is therefore the positive fact promotion writes and attestation never does — an origin `uri:` of `corpus://<container>?stream_id=<n>`. A standalone single-track capture, having no such lineage, keeps whatever its mime declares. **Default-member resolution** (§6.2) still makes the bare op transparent (`?transcribe` on a single-audio MKV needs no `stream_id=`); what changed is that transparency is now a property of *addressing*, not a reason to call a leaf a manifest. The general form of the rule holds regardless of medium, and is stated here rather than pointed at: a **pure envelope** — a transport with no content of its own (zip, tar, gzip, a single-track media container chief among them) — decomposes to nothing once it holds exactly one member; that member is promoted and the envelope is treated as packaging. A **content-bearing** container — HTML, `message/rfc822`, a media container carrying its own chapter marks — never reduces to its member regardless of count: member count says nothing about whether a container is reducible, only a declared mime-schema property does (history §12.20 item 1). The same-video composition — tracks threaded over the shared timeline — is a read-time derived view joining on (lineage, time), exactly as a cross-platform conversation view joins on (ledger identity, time): same tuple, same composition layer, no media-specific logic stored anywhere.
...
*(21)* An **attached location** (§12.1.1) is the third route class: an operator-managed tree whose files stay in place under their own names, resolvable by bare id through a derived location index (§12.9.2) once an attest pass has hashed them (or ingested the tree's presented manifest — §12.1.1 *(24)*). Like the container route, the map is derived and the record never stores its residence; unlike a container, the tree is mutable — so the route serves bytes only while the index's staleness pins hold (§12.1.1), and identity is re-verified in full at every promotion.
...
- **A stored rendering rides a named form.** Steady-state invariant: a record stores a content zone beyond its byte-marks only where a form contract governs it — under the identity contract, a "stored rendering" would merely restate bytes the resolver already derives. (Formless segments *within* a record that carries a form span are the mixed-artifact case and conform — §4.3.2.1. Renderings the 2.x→3.x migrations grandfathered without a form exit through their next pass — history §12.19.)
...
State is **reported, never stored**: health, the queue's pass gate (§8.5), and the ledger's verification (`ledger.md` §13.2) each derive the predicate they need from the record; the touch chain (§4.2.2) remains the provenance trail. *(3.0: the `draft` status retired — the §8.1 tombstone. 3.1: `stub` and `normalized` follow it; a record carrying a `status:` field reads tolerantly — the field is ignored on read and dropped on the record's next write; history §12.19. 3.2: the `authored` predicate follows — and with it the stored editorial pair: frontmatter `title:`/`description:` read tolerantly as the override of §4.2.1 and are dropped by the migration sweep where not deliberately asserted; history §12.21.)*

### 4.2 Frontmatter

The frontmatter (`---...---` at the top of the file) holds **only the bytes-identity header** — four fields *(3.1: `status` retired, §4.1; 3.5: `title`/`description`/`canonical` retired outright, below; v20: `transport` renamed `hash`, `perceptual` retired to the derived hash index, below)*. Everything else lives in body blocks (§4.3) or surfaces as derived views (§9).
...
| `hash` | `<tag>:<hex>` \| list[`<tag>:<hex>`] | no | *(v20; succeeds `transport:`.)* Auxiliary identity hashes beyond the primary blake3 (which lives on `id` and is **not** duplicated here). The tag classifies each value (§2, §7.6): a bare **algorithm id** (`sha256`, `md5`, `blake3-64k`) is a byte-stable, byte-verifiable digest — written at ingest per the declared `residency: record` recipes (§7.9) or added by deliberate flush; a **`<procedure>@<version>`** tag (`html-stampfree@1`, `eml-stripped@2.1`) is a procedure-versioned canonicalized identity — flush-only, never automatic, equality meaning "same content under that procedure at that version." Similarity-class values (§7.9) are never admissible. |
...
*(v20)* **Residency lands here** (§2), and three field fates follow. **`transport:` renames to `hash:`** — the sweep is frontmatter-only (24,716 records, every value byte-identical, one commit; a lingering `transport:` reads tolerantly as `hash:` until swept). The rename fixes a day-one imprecision: the record's actual transport identity always was `id`; this field only ever held *auxiliary* digests — and it repairs the family, since `transport` keeps meaning transport identity exactly where it says so (the members rows, `source_transport`). **Canonicalized identity re-earns record residency** under exactly the condition the 3.5 retirement set — "a record needing a content-identity beyond its bytes will re-earn the field with a strategy that works" — but as tagged values in `hash:`, not as the old field: procedure-versioned tags only, flush-only writes, and the first procedure (`html-stampfree@1`, §7.9) canonicalizes nothing but corpus-injected bytes, so it is structurally incapable of the cross-document collapse that killed `canonical:`. The retired `canonical:` field itself stays retired. **`perceptual:` retires** (record-scope here, per-segment in §4.3.2.2's header table) with a fleet population of **zero** — the opt-in knob was never exercised; fingerprints are similarity-class and live in the index only (§7.7, §7.9). It reads tolerantly on a legacy record.

*(3.5; canonicalized identity has since re-earned residency as tagged `hash:` values — v20, above — while the `canonical` field itself stays retired)* Three fields retire from this table. **`title` and `description`** — the optional overrides 3.2 left behind — are gone: an override is by definition the record asserting a display value of its own, which §4.2.3 no longer admits from any layer. The derived pair resolves from attested byte-facts or resolves empty, and empty is an honest answer. **`canonical`** is gone with the mechanism (§7.1): the strategy was disabled on the write path in 2026-06-28 after `blake3-canonical-pdf` was found to collapse unrelated scanned PDFs, no shape was ever settled for the HTML population that carried most of it, and nothing has read the field since. A record needing a content-identity beyond its bytes will re-earn the field with a strategy that works; until then storing a hash nothing computes and nothing reads is stale state that reads as live.
...
- **`forbidden`** — an inherent sequence, **or** an address space already total. Both make absence wrong, for opposite reasons: a sequence has no whole to name, and a total space (HTML under 3.6) has an honest address for every region including the outermost, so absence is unnecessary rather than untrue. *(`application/pdf` — `page=`; `application/mbox` — `msg=`; `video/*` — `time_range=`; `text/html` — `el=`.)* A one-page PDF is **not** an exception: `page=1` is a true statement about a real unit, so nothing is fabricated by requiring it.
...
***(3.13) The frame axis is the sequence's road to conformance.*** 3.10 stated where an animated image could not go (absence over a multi-frame artifact) without giving it anywhere to go — `bbox=` was the image space's only axis, and a fabricated `bbox=0,0,1,1` would have looked correct forever where a stated non-conforming record does not (#124). The animation-capable image schemas now declare `frame=` (§7.1 `address_scheme`; §6.2 for the grammar): a rendering of one frame addresses `frame=<N>`, and a rendering that holds across the whole sequence — a burned-in caption the animation never changes — addresses the **inclusive span** `frame=<A>-<B>`, which is an *assertion*, checkable frame by frame, never an omission and never a single frame dressed as the truth about all of them. The two shapes are content judgments, not grammar: a changing screen flattened into one text is exactly what the 3.10 gate exists to catch, and it decomposes into single-frame segments; an invariant caption is *most truly* rendered over its full span (the measuring cases: a 226-frame changing terminal screen vs an 8-frame static-caption animation, #124 2026-07-30). `frame=` chains frame-then-region (`frame=3&bbox=…` — a region *of* frame 3; the reverse order is a lint error), and its stored values are gated by `address-frame-invalid` against the same attested `frame_count` the whole-address rule compares — compare, never decode, both rules alike.

**The gate compares an attested count; it never decodes.** The count is stamped at ingest attestation (§8.3) alongside the sequence counts the bearing types already carry — `page_count`, `message_count`, `member_count`, `card_count` — so `whole_address_count` names an existing convention rather than inventing one. A gate that had to open and decode the artifact would report red when a decoder was absent rather than when a record was wrong, which is not a validity check but a report on the checking host: the same substitution that makes an undecodable SVG carrier look like a bad address (history §12.34).
...
| Field | Description |
|---|---|
| `perceptual` | *(v20: retired — a fingerprint is re-derivable from the artifact's bytes, so it fails §2's residency rule; when the §7.7 knob resolves on, fingerprints land in the derived hash index keyed by segment address (§12.9.1). Reads tolerantly on a legacy record; the fleet's population was zero.)* |
...
*(3.4: this retires the raw-blake3 wikilink/embed forms `[[<blake3>]]` / `![[<blake3>]]` and the functional-URI forms `[[corpus://…]]` / `![[corpus://…]]` from segment bodies. Both were specified, both were enforced by lint, neither was ever rendered by anything, and no record in either hub ever used them — history §12.26. Unaffected: the origin block's lineage `uri: corpus://<container>?<address>` on a promoted record, which is capture **history** and is never consulted for byte lookup, §8.1/§12.15.)*
...
***The grain of the deconstruction is the address, not the segment.*** A leaf may carry several segments at one address — identity is (`opener-id`, `address`), so a `text` and a `text/data-table` describing the same region are distinct segments legitimately sharing one (§4.3.2.2) — and a placement naming that address imports **all** of them, in the leaf's order. This is a deliberate coarsening: a placement carries an address and no opener, and giving it one would make the parent name *which representation* of a region it wanted, which is a judgment about the member's content and therefore the leaf's to make (§4.3.2.4's opening). Until a real case needs that granularity, one address is one placeable unit — which also keeps *exhaustive* countable against something the leaf already partitions itself by.
...
Lifting the whole-record section's sibling prohibition (§4.3.2.1) removes the reason. A trailing form span over the rail's region renders those links as what they are: faithful body content, in the source's own order, with the source's own labels, addressed on the artifact's own axis, and citable. Nothing is lifted, nothing is stored twice, and the cross-record edge remains what it always was — a read-time resolution of the URL (§12.4.7), self-healing as targets are captured or removed. **Trailing** is the ordering rule, not a preference (§4.3.2.1): the rail is the page's frame, not its subject. And the rail is rarely the only such region — a page that carries one usually carries a breadcrumb too, and the two are one entry set under one shape, so they take **one** span whose segments render each region at its own address, in source order within the span. The span's envelope is then the ordered address list its children derive (§6.1.1) — the honest form for a claim over disjoint regions, and the one that cannot overlap the content span beside it. *(2026-08-08, #89: that form is **`form/nav`** — the navigational link set, crumb trail + link rail. This paragraph originally prescribed `form/index`, and the overload proved real: a page whose entries ARE its content and the trailing span framing it shared one bare spelling, so §4.3.2.1's adjacent-same-form rule merged them the moment legacy fields stopped distinguishing them — 584 records refused by the restoration verbs. The shapes were never the same: an index's entries are the content; a nav span's entries point at content from beside it. A form-library split, not a grammar change; the freeze stands.)*
...
#### 4.4.5 The citation ladder *(retired 3.5 with the reference block — §4.3.3.3; a link's position is wherever the faithful body renders it)*
...
*(3.4: the raw-blake3 wikilink/embed forms `[[<blake3>|link text]]` / `![[<blake3>]]` are retired — specified, linted, rendered by nothing, and used by no record in either hub. History §12.26.)*
...
**The space names elements, so text in no element has no tight address.** *(3.9, stated 2026-07-29 — a limit, recorded rather than worked around.)* Making every element addressable does not make every *region* addressable. Prose emitted as bare text nodes between `<br>`s in a flat `<div>` — a shape a great deal of application markup produces — belongs to no element but its container, and the container is routinely the page's whole content. Three forms and none of them fits: the container's own path is true but names far more than the prose; a **sibling range over the bracketing elements** is true and still names everything between them, which is the rest of the page's prose too; and a list names the landmarks the prose sits between rather than the prose. *(Measured when the question was forced: of the segments whose prose has no element of its own, **not one** yielded a sibling range tight enough to survive a container-to-body size check — the bracket span always covered many times the segment's own text. The range can say where the prose sits; it cannot say it tightly.)*

This is not a gap to be closed by a fourth range form. A "from character offset to character offset" address would name a region that is not a structural thing, which is exactly what the retired `el=<N>-<M>` did and exactly how over-claiming envelopes arose — and it would re-point on any reparse, losing the permanence the whole amendment exists to buy. **The honest response is at the record layer, not the address layer**: where several segments' prose shares one container and none has an element of its own, they are one region of the artifact and belong in **one segment** addressed at that container (§4.3.2.1 — a span that is a subtree *is* that subtree's own address). What is lost is the prose's internal ordering against the pictures interleaved with it; what is gained is that the record stops claiming a picture's address for a paragraph. Migration: history §12.33.
...
| `frame=<N>` \| `frame=<A>-<B>` *(3.13)* | animated image / video | image / — | **Polymorphic** (see below). On an **animated image** (GIF, animated WebP/AVIF): a **1-based ordinal** selecting one frame of the sequence, bounds-checked against the attested `frame_count`; leads its chain, so `frame=3&bbox=…` names a region *of frame 3*. The inclusive span form `frame=<A>-<B>` is a **stored-address assertion** (§4.3.2.2 — a rendering that holds across every frame in the span); it names no single byte surface and does not materialize. On a **video**: a timecode (seconds or `HH:MM:SS`) rendering the frame at that timestamp to an image — the drafter's positioning-marker address (§7.1 `video`). A stored image-side value that breaks the ordinal grammar, exceeds the attested count, or trails a region param is a lint ERROR (`address-frame-invalid`). |
| `bbox=<x>,<y>,<w>,<h>` | image / spreadsheet | image / cell-range | Crop a relative region (image: floats in `[0.0, 1.0]`, origin top-left) or narrow a worksheet (spreadsheet: an A1 range, e.g. `bbox=B2:G30`). Polymorphic — see below. The 3rd and 4th values are a **width and a height, not a second corner**: `x+w` and `y+h` must each be ≤ 1. A stored region-op value that breaks this grammar is a lint ERROR (`address-region-invalid`) — the region grammar had been enforced only at render time, so an address nobody resolved could carry pixel values or corner coordinates and pass every gate (history §12.24). |
...
Parameter value grammar may be media-type-dependent; the resolver dispatches on the source artifact's type. In particular `bbox` is **polymorphic** — relative floats in `[0.0, 1.0]` when cropping a rendered image (an image artifact, or a `page=` render of a PDF), and a spreadsheet cell range (e.g. `bbox=B2:G30`) when narrowing a worksheet region — so the same token does not collide across media types. `frame=` *(3.13)* is polymorphic the same way: a **1-based ordinal** (or inclusive ordinal span) on an animated image, whose frames are a sequence with no timebase a reader can trust (GIF delays are per-frame and advisory), and a **timecode** on a video, whose timeline is the medium's own axis — the physics of each medium picks its reading, and the token does not collide. Pure **address selectors** that locate a region without transforming it — `sheet=<name>`, `el=<path>` (§6.1.1), and any others — are defined by each media-type schema (§4.3.2) and are not enumerated here; §6.2 lists only the parameters that produce a derived view.
...
**Timeline inheritance** *(3.11 — RETIRED at 3.12; kept here because the measurement behind it is the argument for what replaced it)*. The mechanism was: a **timeline op** (`frame=`, `time=`, `time_range=`, `scenes=`) addressed at a promoted **video** stream leaf resolves through the leaf's containment lineage, against the container, with the leaf's own `stream_id=` composed in. It was modelled on the case §1.2 states for chapter marks — *"tracks inherit the container's chapter marks at read time through lineage — never copied, because the track's own bytes do not carry them"* — applied to the timeline the marks sit on, for what looked like an identical reason: **a bare Annex-B video stream carries no timing at all.** No timescale, no duration; a decoder imputes a frame rate from in-band parameter-set timing where present and from a default where not.

**Why it is retired rather than fixed.** The redirect was a correct answer to the wrong question. It treated the leaf's missing clock as a fact to route around, when it was a **symptom of the wrong decomposition target**: 3.12 makes a promoted stream a single-track container of the source's own family (history §12.20 item 1), which carries `mvhd`/`mdhd` timescale and sample tables and therefore reports true timing and seeks natively — measured at `30/1`, `151.300000`s and 4,539 frames against a container reporting exactly the same, where the Annex-B extraction of the same track reported no duration at all. A leaf with its own clock needs no lineage walk, so every timeline op resolves on the leaf directly and the composition below is machinery for a condition that no longer arises. The survey that follows stands as the record of *why* the elementary form was insufficient; it is no longer a description of how anything resolves. The two leaves promoted under the superseded rule are re-promoted with it (history §12.37), so no record depends on the retired path.

That is not a theoretical gap, and it was measured across **8 rostered containers** (4 h264, 4 hevc; 4.4s–616.5s; phone captures, a screen recording, 4K clips, a long CFR upload) rather than argued from one.

**The leaf's duration is categorically unavailable — 8 of 8.** `ffprobe` returns no duration *field* (absent, not `N/A`), and a per-frame timestamp query returns nothing for every frame of every leaf, h264 and hevc alike. That alone settles the final span: nothing on the leaf can close it.

**The leaf's reported frame rate must not be read at all.** Every one of the 8 leaves reports the identical pair `r_frame_rate=1200000/1`, `avg_frame_rate=25/1` — a constant placeholder, present regardless of the source and present even on the leaves whose real decode timeline turned out to agree with the container exactly. It is uninformative in both directions, and computing a correction factor from it produces a number about nothing.

**What a real decode shows is that agreement is the minority case.** Comparing `scenes=` cut lists container-side against leaf-side: **2 of 8 matched exactly** (both h264 whose SPS carried a clean VUI-declared round CFR rate — one of them the pilot deck (history §12.20), 32 of 32 cuts identical to the microsecond); **3 diverged by an exact constant factor** equal to the source's real rate over ffmpeg's flat 25 fps fallback (1.2×, 1.2×, 2.4× — all three hevc); **2 diverged by a non-constant, growing amount** that no single factor can invert (an h264 with an irregular per-frame gap in the container drifting 1.07× → 1.17× over 30 seconds, and an hevc screen recording with genuine variable frame timing spreading 2.24×–2.35×); 1 was inconclusive. **All four hevc leaves fell back to 25 fps, including a clean-CFR one** — and hevc is this corpus's dominant video codec, being phone-native. So the leaf's own timeline is wrong for most real records, sometimes correctably and sometimes not, with nothing on the leaf to say which.

**Seeking fails even where detection agrees**, which is the sharpest reason not to treat the matching cases as safe. On the pilot leaf — one of the two exact matches — `ffmpeg -ss 30` returned success and a **zero-byte image**. Input seeking and a filter graph's fabricated presentation times are not the same mechanism, so a leaf can produce the right cut list and still fail to render a frame at one of its own addresses. A second of "leaf time" is not a second of the timeline a stored address was computed in, and the stored address is the thing that must not move.

Three consequences, each of which the alternative gets wrong. Addresses on a leaf and on its siblings are in **one shared timeline**, so a video leaf's scene spans and its audio sibling's transcript spans can be read against each other — which is the whole point of §1.2's shared-timeline composition. Cut boundaries are measured where they will be resolved, so a `time_range=` resolves to the frames it names rather than to a scaled approximation. And the leaf stays a *leaf*: nothing is copied down onto it, so the container remains the single place the timeline is defined. As with every lineage chain, the container is blake3-pinned and immutable, and an absent one fails loudly at resolve rather than resolving against a substitute.

Two ops are deliberately **excluded**, and both exclusions are load-bearing. `format=` addresses no timeline — an encoding change is a leaf's own playable rendering, so it stays a leaf operation. `transcribe` stays on the leaf because an **audio** elementary stream is not in the video stream's position: ADTS and the Opus pinned framing are self-framing at exact fixed per-packet durations derived from the sampling rate, so an audio leaf's timeline is well-defined from its own bytes. Redirecting it would also be actively destructive — a container has no video→text transform, so the chain would route through `extract_audio`, which re-encodes to mp3, and the transcript would be of a lossy derivative rather than of the member's own pinned identity bytes. **Reach for the container only when the member's bytes are not self-sufficient.**
...
- `framing` *(3.12)* — **the stamp that admits an engine into the identity path.** A promoted stream leaf is muxed, not reframed (history §12.20 item 1), so the producer is a versioned tool rather than pure byte-work. The tool, its version, and a **sample count** are attested onto the leaf's artifact block — the same shape and the same purpose as `addressing:`'s `parser` + `elements`:
  ...
  The count is the cheap self-check the other two stamps already use: a consumer whose own demux finds a different sample count **knows it disagrees**, rather than silently trusting bytes a different engine version produced. It is deliberately the *sample* count and not a byte length, because that is the quantity the corpus's own reader (`corpus.streams`, which parses the sample tables without an engine) can verify independently of how any muxer chose to lay the boxes out — an oracle that does not share the producer's failure mode. `version` records what a re-derivation must match to be a refresh rather than a re-identification; a differing version is not by itself a defect, it is the fact that makes a byte change **explainable instead of mysterious**.

  This is the slot for it rather than the record's `touch:` list, and the distinction is load-bearing: `touch:` is provenance history — the transforms a record has been through — while a stamp is an **attested fact about the bytes as they now stand**, carrying its own disagreement check. An engine version in `touch:` would document the past; in `framing:` it constrains the present.

- `cutting` — **the stamp, not a schema key.** The mime schema declares the *default*; the resolved strategy, its parameters, and its result are attested onto the **artifact block** of the stream leaf, exactly as `addressing:` stamps parser and element count (§6.1.1):
  ...
  `cuts` counts the resulting **spans** — the addressable things — not the detector's boundary points. N boundaries inside a timeline yield N+1 spans, so the values above are the pilot deck's real ones: `scenes=0.3` reports 32 boundaries over its 616.53s, which close into 33 addressable spans (history §12.20 OQ1). The span count is the right thing to attest for the same reason `addressing:` attests an **element** count rather than a parse depth: it is one per stored address, so a consumer comparing it against the record's own markers is comparing like with like. It also means an empty count is never legitimate — a timeline with no detected boundary is one span, not zero — which is what lets a zero read as malformed rather than as "found nothing".

  Three rules govern it, and each answers a way the key could otherwise rot.

  **Resolution happens once, at promotion, and the result is stored.** A stream leaf is reached by containment, so its own origin is `corpus://<container>?stream_id=<N>` — capture lineage, which §12.15 is explicit is *never* a lookup route. The strategy therefore cannot be resolved by a reader walking to the container's overlay. `promote` already holds both records (it is reading the container to stream the member's bytes), so it resolves *mime default ← container's origin overlay* there and writes the answer down. The leaf is thereafter **self-describing**: no walk, no dependency on a second record, and the resolution is auditable from the record itself. This is not the stored-derivable-fact 3.4 and history §12.26 both refused — what is written down is a **resolution**, which is not recomputable from the leaf's own bytes at all.
...
- `derived_hashes` *(v20; succeeds `transport_algos`)* — list of **recipe ids** (§7.9) this format adds to the corpus-wide default set (§7.9's layer model: default ∪ mime ∪ origin overlay, additive union — a declaration adds, never suppresses). Destination follows each recipe's residency class (§2, §7.9): a byte-stable recipe with `residency: record` lands in the frontmatter `hash:` **and** the index; every other recipe lands in the index only. A schema still declaring `transport_algos` reads as `derived_hashes` with each bare algorithm name as a `residency: record` byte-stable recipe — which is exactly what the old key did, its values landing in the renamed field.
- `ref_adapter` (optional) *(21)* — names the **reference-dataset format adapter** (`ledger.md` §6.5) that serves entries out of artifacts of this format (`zim`, `osm-pbf`). Schema-first deployment metadata in the tenancy mold (`ledger.md` §6.4): the corpus itself never consumes it — the reference-dataset resolver reads it to derive a registered dataset's adapter from its mirror record's mime, so manifest registration can omit `adapter:` (`athenaeum.md` §2.3; an explicit manifest declaration still wins — the bootstrap and override path).
- `replicas` (optional) *(21, sketched — enforcement lands with the replicate verb)* — the format's **redundancy floor**: the minimum number of distinct locations (§12.1.1) that must hold any artifact of this format. An origin overlay MAY declare the same key for a source whose bytes are irreplaceable regardless of format (a producer export that can never be re-pulled); the resolved floor is the **max** across layers — the additive-union instinct of §7.9: a declaration strengthens custody, never weakens it. Consumed by health's under-replication signal and the planned `corpus replicate` verb (§12.1.1); the corpus itself enforces nothing at write time, and an undeclared floor is 1 (the bytes exist somewhere — §2's baseline contract).
- `canonical_strategy` — *(v20: the key retires outright; it had been dormant since 3.5. Canonicalized identity is now a §7.9 recipe class, its values re-earning record residency as tagged `hash:` entries — §4.2.1.)* The key's ambition — **content-canonical** identity, "the same article reached from two URLs" — remains retired under the 3.5 conditions: a canonicalization that cannot conflate a scanned page with an empty one, and a settled content region for HTML, neither of which exists. What v20 ships instead is the deliberately narrower tier: **capture-invariant identity** (`html-stampfree@1`, §7.9) canonicalizes only bytes the corpus's own pipeline injected, so it can never merge distinct source documents — the failure that killed `blake3-canonical-pdf`. A future content-canonical, if it clears the 3.5 bar, enters as another versioned §7.9 recipe; this key stays retired either way.
  - **The 3.5 retirement record, kept for its conditions.** The strategy was disabled 2026-06-28 after `blake3-canonical-pdf` was found to hash only per-page extracted text — every text-empty (scanned) PDF canonicalized identically and unrelated scans were silently merged, the duplicate's bytes discarded. The disable did not sweep: 7,210 records carried a dead `canonical:` for two years while the spec asserted none did. v20's mechanics answer both halves of that failure: an unproven recipe lives in the regenerable index (a failed strategy is a config deletion and dropped rows, no fleet sweep), and a value that does reach a record goes **version-tagged** (§2), so a superseded procedure reads as superseded, never as live.
...
- `derived_hashes` (optional) *(v20)* — recipe ids (§7.9) this producer's records add to the effective set, the third layer of the additive union (default ∪ mime ∪ origin; §7.9). Producer knowledge on the producer's grain: a host- or export-specific canonicalization (`eml-stripped@2.1`-shaped) rides here exactly as strip declarations do (§12.3.13), never on the mime schema.
...
*(3.9)* **Regions nest, and the innermost declaration governs.** A byte of the artifact belongs to **exactly one** region: the declared region with the smallest span containing it. Everything else follows from that one sentence — a `framing` or `never` region inside a `subject` envelope is framing or never *for its own bytes*, and a `subject` envelope inside another contributes its content once, not twice.

The rule is not a refinement, it is what makes the declaration mean anything at all. Modern application markup is a tree of custom elements, so a page's outermost content element contains its breadcrumb, its vehicle header **and** its cross-link rail; reading that element as `subject` claims all three as the page's subject while the overlay's own next three rows say they are not. Two readings of the same declaration set, both defensible from the text, differing by tens of thousands of anchors — which is exactly what a machine-readable declaration exists to prevent. *(Measured on one host at the amendment: a wrapper read as an envelope scored 39,931 flattened anchors where the innermost reading scores 30,677, and the gap was almost entirely a second region's restoration backlog counted as a first region's defect.)*

Two consequences worth stating, because both were found the hard way:

- **Containment is a property of the artifact, not of the declaration order.** The same two selectors may nest on one template and sit side by side on another, so a reader resolves containment **per record** against the bytes. Declaration order remains what §7.2 already says it is — the order framing regions take in the trailing span — and never implies nesting.
- **An overlapping-but-not-containing pair is an error, not a tie to break.** Two declared regions whose spans interleave describe a markup shape no DOM produces, so the declaration is wrong; report it (§7.2's declaration gate) rather than picking a winner. Silence is not `subject`, and a coin toss is not a judgment.

*(3.8)* An origin overlay MAY declare **exemplars** — handcrafted records of this same origin that show a shape rather than describing it:

- `exemplars:` — `[{record, content, shows, address?}]`.
  - `record` (blake3) — a record of **this same origin**. Cross-origin exemplars are forbidden, and not merely by convention: shape judgments are origin-specific (a repair site's print residue is nothing like an mbox's), and the same-origin rule also keeps an exemplar inside one hub, so tenancy holds without a second mechanism.
...
The `<algo>:<hex>` value records which algorithm produced it, so an index row self-documents how its record was fingerprinted *(v20)*.
...
1. the **default set** — corpus-wide, applied with no declaration: `sha256` and `md5` (`residency: record` — sha256 the interop standard and the fleet's existing convention; md5 for store-side verification against object-store metadata, S3 single-part ETags / Azure `Content-MD5` / GCS `md5Hash`, an integrity audit with zero egress) plus `blake3-prefix-ladder` (index-only — the grown-export screen joins on producer filenames, which no mime grain predicts);
...
|---|---|---|---|
| `sha256` | byte-stable, `residency: record` (default set) | sha256 over the artifact bytes, unmodified. | Interoperability with external checksum ecosystems — producer-published digests, replacement-copy matching for lost bytes (§12.17). The direct successor of `transport_algos: [sha256]`; existing fleet values are already exactly this. |
...
| `draft` | *(retired in 3.0.)* The stage's three duties split: fact-stamping → ingest **attestation** (above); content extraction → resolver **derivation ops** (§6.2); body-writing → the normalize pass. A `status: draft` record reads tolerantly as a just-attested record (history §12.18). | — |
...
|---|---|
| `id`, `transport` — byte-intrinsic. | `perceptual` (record-scope). |
...
- **Deterministic stream extraction** *(3.0; answered at 3.12)*. Track promotion from media containers (§1.2) requires member-byte determinism; the mbox precedent achieves it by pinning extraction semantics on the mime schema, and the same discipline applies here (sample data in decode order per the container's own tables; HEIF `item=` payloads likewise). 3.0 named *"pinning a demuxer's output across tool versions"* an open engineering item and 3.12 answers it — not by keeping engines out, which proved to be a rule that refuses records rather than one that guarantees bytes, but by **pinning the invocation and attesting the producer**: `-c copy -fflags +bitexact`, a `framing:` stamp carrying muxer, version and sample count (§7.1), an independent sample-table oracle that does not share the producer's implementation, and a suite canary against a fixed hash. Cross-version stability is the flag's designed contract and is checked by the canary rather than assumed. Content-addressing plus continuity-gated supersession (§12.8) remains the net if a pinned extraction ever shifts bytes.
...
│   ├── locations.db         attached-location index (§12.9.2, v21)
│   └── refidx/              ref-dataset sidecar indexes, by mirror blake3 (v21; ledger.md §6.5)
...
#### 12.1.1 Artifact locations *(21)*

The co-located `artifacts/` tree is the **default location**, not the only one. A corpus MAY declare additional locations in its deployment configuration (`corpus.toml` — untracked, machine-specific; the reference shape below), and byte resolution (§2) falls through them exactly as it falls through containment — another route class, invisible to records:
...
ingest = ["application/x-openzim"]   # optional placement policy: these formats land here
                                     # *(22)* `ingest = true` instead makes this the DEFAULT
...
ingest_origins = ["download.geofabrik.de"]   # *(23)* origin claims: artifacts whose minting
                                     # origin matched one of these origin overlays (by
                                     # overlay id, §7.2) land here — beats format claims
...
manifest = true                # *(24)* presented manifest: the host's own residence
                               # scanner maintains <path>/.athenaeum/; attest reads
                               # it instead of walking the tree
...
- A **store** location is a content-addressed `<shard>/<hash>.<ext>` tree the corpus itself writes — §12.1's layout at another root. Ingest placement policy MAY direct new artifacts of declared formats there instead of the co-located tree (tens-of-GB mirrors belong on bulk storage, not beside the repo) — *(22)* or, with `ingest = true`, ALL new artifacts: the most specific claim wins (a format list beats a default; the co-located tree is the fallback when no location claims the format), and at most one location may declare the default. *(23)* A location MAY also claim **origins** (`ingest_origins`): a list of origin overlay ids (§7.2) matched against the overlay the minting origin resolved to — no second pattern syntax; the overlay IS the origin's identity. Precedence, most specific first: origin claim → format claim → default → co-located; within one axis, declaration order breaks ties. An artifact minted with no matched origin overlay can never satisfy an origin claim; `put`/`ensure_local` address every store location symmetrically. The existing object-store backends (§12.9.1's remote store; the azure/s3 extras) are store locations with a remote transport — **rclone** generalizes that transport to anything it reaches (SMB shares, B2, SFTP, …), an optional extra in the same guarded mold as every other backend.
- An **attached** location is an operator-managed tree the corpus reads but never rewrites: files keep their own names and paths — never renamed, never moved into content-addressed form (*(23)* **adoption**, below, copies them into one, leaving the tree untouched). An **attest pass** (`corpus location attest <name>`) streams and hashes the tree (hash-while-in-hand — §12.9.1's principle: the pass opportunistically fills the derived hash index too) — *(24)* or, where the location **presents its own manifest** (below), ingests that instead of walking — and writes the **location index** (§12.9.2): blake3 → `(location, relpath, size, mtime)`. From then on those files resolve by bare id like any artifact. This is the containment model generalized: an attached location is, functionally, a container whose manifest is the location index — members stay in place, resolution routes through a derived map, and **promotion mints records without moving bytes** (§8.1's shape: identity verified by the full hash at mint; the origin block records the location provenance under its scheme family, §7.2 — and, as with container lineage, that origin is history, never a lookup route).

Resolution order is co-located store → other store locations → attached-location index → member index (containment) → remote hydration; any route yields identical bytes (§2), and a resolver may take any.

**Move semantics** *(22)*: residence is mutable because it is invisible (§2). `corpus location move` relocates a record's **standalone copy** between store locations — the co-located tree counting as one — by copy-verify-then-remove: the bytes are copied into the destination's content-addressed path via a temporary name, the full blake3 is verified against the record id before the rename unveils them, and only then is the source copy removed — at no instant is the id unresolvable by the route being replaced, and a failed verify leaves the source untouched. Attached-location files never move (operator-managed, above — adoption is the custody-transfer path, not a move); a containment-only member has no standalone copy to move — materializing one is `replicate`'s job, not `move`'s. A destination already holding the file is idempotent success: verify, then remove the source. The remove half owes redundancy floors the same honesty as `rm`: a move is location-neutral in copy *count*, but where a floor counts *distinct* locations a same-location dedupe is not. *(23)* `move` MAY take a **batch form** — every record whose standalone copy resides at a named source location — with per-record outcomes, continuing past failures and reporting them, in `promote --all-matching`'s mold.

**Adoption** *(23)*: `corpus location adopt <attached> <dest>` transfers a file's **primary custody** from an attached location to a store — owner-initiated, never implicit, and **non-destructive by default**: the destination store gains a copy under `move`'s contract (streamed to a temp name, full blake3 verified against the record id before the rename unveils it), and the attached original stays exactly where it is, untouched. From then on every consumer is served the store copy — the resolution order (store locations before the attached index, above) shadows the attached row with no further mechanism — while the row itself persists honestly in the location index: the shadowed copy is invisible in use but **surfacable** (a health signal lists records holding both a store copy and a current attached row — dedup/reclaim candidates; and two copies are genuinely two, counting toward a redundancy floor). Removing the original is a separate explicit act (a reclaim flag, or the operator's own management of the tree) — never adoption's default, and for a read-only attached tree never possible at all. Files without a promoted record are refused (adoption transfers custody of a record's bytes, it does not mint — promote first); a stale index row is refused exactly as promotion refuses it (re-attest first).
...
**Presented manifests** *(24)*: an attached location on a remote host MAY declare `manifest = true` — the tree **presents its own manifest**, maintained by the **residence scanner**, a shared tool run *on the host, against the local filesystem*. The locality is load-bearing twice over: hashing happens at disk speed with nothing crossing the wire but the manifest itself, and the scanner's change model — identity `(dev, ino) → (size, mtime) → blake3` — is only sound where inodes are real (through an SMB/NFS mount they are synthetic). Under that model an unchanged stat tuple is never re-hashed, a rename is a path row onto a known identity (zero re-hash), hardlinks collapse to one hash, and an interrupted pass resumes at per-file granularity — so a scheduled incremental run stays cheap forever after the cold pass. The manifest lives at the location root (reference shape `<path>/.athenaeum/`: the scanner's private working state beside a **published** manifest database — a complete, checkpointed copy unveiled by temp-then-rename at each generation close, so a reader over the mount only ever sees a whole generation, never a write in flight); its **byte format is the scanner's own versioned cross-language contract**, deliberately outside this spec so the writer is swappable without a spec change. Attest on a presenting location reads the manifest instead of walking — a whole share becomes attestable over a mount that only ever transfers the manifest, retiring the tree-walk scale limit — and the manifest directory itself is infrastructure, excluded from indexing as content. Location-index rows carry their **provenance**: `computed` (attest streamed and hashed the bytes itself) or `presented` (a host claim the corpus has not independently observed). Trust is the standing rule — **the index proposes, the hash disposes**: presented rows serve resolution under the same staleness pins as computed rows, and every act that mints or replicates identity verifies the full blake3 exactly as before — promotion at mint, adoption during its copy — so a false claim can at worst misroute a read, never mint or replicate a wrong identity. A deployment wanting more than the pins MAY re-verify presented rows with an ordinary walking attest; bit-rot scrub duty on the tree belongs to the scanner on the host, where the bytes are local.

**Catalog metadata** *(25)*: the manifest grows from residence map to **light catalog**. An identity carries the **universal stat facts** — size and mtime (the pins, as ever), ctime, mode, and birth time where the filesystem has one (nullable: creation time is near-universal, not universal) — refreshed by the walk whenever they drift, at no cost beyond the stat the walk already performs. `atime` is deliberately excluded (noise the scan's own reads would perturb); owner ids are deferred until a need names them. One derived fact rides along: a **mime claim**, sniffed from the leading bytes once per new identity while the bytes are in hand at hash time (a bounded head-read on the native-hash path), so format questions — what formats reside where, which artifacts would a placement claim route — are answerable **without touching bytes over the wire**. The claim vocabulary is claims all the way down: like the presented hash it qualifies, a mime claim is advisory — ingest's own detection (§12.3.2) is authoritative and undisturbed, and nothing normative may depend on a claim being present or right. The location index imports the mime claim beside the pins (§12.9.2); the remaining catalog facts stay queryable in the location's cached manifest rather than being copied forward.

**Residency query** *(24)*: `corpus locate <blake3>` answers, for a bare hash, **where those bytes reside** — across the co-located store, store locations, the location index (computed and presented rows alike), the member index, and remote objects — and whether a record exists for them. It is read-only and works for hashes the corpus has never minted; that is the point: an external hash can be checked against every attached residence without moving a byte over the wire, making locate the natural precursor to promotion and adoption. A hash with **multiple residencies** is the dedup surface: locate lists them all; health's shadowed-copy signal (adoption, above) is the special case where one residency is a store copy, and the general case — the same bytes at two paths, on one tree or two — surfaces the same way: candidates for the operator's reclaim judgment, never automatic deletion.

**Route preference** *(25)*: §2's license — any route yields identical bytes, and a resolver may take any — has always permitted choosing well; this names the signal. A location MAY declare `cost = N` (any non-negative integer; lower is nearer). Undeclared, cost defaults by class in today's resolution order — the co-located store nearest, then store locations, then attached — so a zero-config deployment behaves exactly as before; a declaration reorders *within* the standing route classes (a remote store location declares itself costlier than a local one; a remote attached mount costlier than an in-chassis tree), it does not promote a route class past identity's stronger guards. When bytes must actually move — resolution's local materialization, adoption's source read, `replicate`'s future top-up — the resolver takes the **cheapest residency whose pins verify**, falling through to the next on staleness exactly as it falls through routes today. The same declaration serves the dedup economics: locate orders a hash's residencies by cost, and replicate's planner (below) reads it as "copy from the nearest, toward the floor."

**Redundancy floors** *(21, sketched)*: with locations, "how many places hold these bytes" is countable. A mime or origin overlay MAY declare `replicas: N` (§7.1); a health signal counts distinct locations holding a route to the record's bytes — a standalone copy in any store location, a container residence, a remote object, an attached-location file each count once — and reports records under their floor. `corpus replicate` (planned) tops up by copying to a store location; `rm` names the floor a removal would break. Nothing enforces at write time — ingest with one copy is still ingest; the floor is a standing health obligation, not a gate.
...
The hash families of §2 land at different destinations by residency class (§2, §7.6 encoding) *(v20)*:

- **blake3 of the bytes** — always, at ingest. The artifact's `id` (bare hex): identity and filename stem.
- **Byte-stable recipes** (§7.9) — computed at ingest from the resolved union (default set ∪ mime ∪ origin overlay): `residency: record` recipes (the default set's `sha256` + `md5`) land in frontmatter `hash:` *and* the index; the rest (the default set's `blake3-prefix-ladder` grown-export screen) land in the index, record-flushable on demand (§12.9.1). *(v20: generalizes `transport_algos` → the renamed field; values are unchanged by the rename.)*
- **Procedure-versioned recipes** — `html-stampfree@1` capture-invariant identity: index-first, reaching frontmatter `hash:` only by deliberate flush under its full `<procedure>@<version>` tag (§4.2.1, §12.9.1). Content-canonical stays retired under the 3.5 conditions; the cross-URL content-dedup fold stays inert until a strategy clears that bar.
- **`perceptual`** — atom fingerprints (image pHash, text simhash, …), **opt-in and schema-gated** (§7.7); default off. *(v20)* Procedure-versioned **similarity-class** (§7.9), index-resident: keyed by segment address on multi-atom records, record-scope on single-atom ones; never record-flushed — a similarity value is not an identity.
...
Ingest emits the record — the artifact's proxy, complete at birth (§4.1). The frontmatter carries only the bytes-identity header — `id`, `hash:` (the declared `residency: record` byte-stable recipes, §7.9 *(v20)*), `touch: [<pkg>.ingest@<v>]`; *(3.1)* no `status` field; *(3.2)* no editorial fields — the display title/description derive from the role-marked attested and sidecar-lifted fields the same ingest just stamped (§4.2.3), so the proxy is presentable the moment it exists. Everything else lands in body blocks:
...
**Title is derived, never authored** *(§4.2.3)*. The frontmatter `title` (like `description`) stays empty through ingest; the display title resolves mechanically from the block-level role-marked candidates — the artifact block's bare `title`, or an origin `ytdlp_title` (the `ytdlp_` prefix survives because the origin opener names the source record, not the tool; §4.2.1) — no authoring pass, normalizer or otherwise, is involved. A media capture's title candidate routes to the origin's `ytdlp_title` (an A/V artifact block carries no `title`); for display, `records.title_for` reads the frontmatter `title`, falling back to the artifact `title`, then `ytdlp_title`. After a successful lift, ingest deletes the sidecar; enrichment is one-shot (re-capture to restore — the lifted fields already persist on the record).
...
- **`corpus continuity <A> <B>`** proves whether record `A`'s addressable content is preserved in `B`: per `path=<member>` it is byte-identical, *contained* (A is a prefix B extends — the append-only case), *diverged*, or *absent*, and `contains_a` is true iff every unit is preserved. This is the supersession safety check — the *same* test answers "may B replace A?" (keep the more complete version: a re-captured session that grew supersedes its predecessor, while a compacted one must not clobber the fuller copy) and "may a citation of A be rewritten to B?" (`ath ledger supersede`, `ledger.md` §13.3). No synthetic stable id is minted: identity stays the blake3, and continuity carries citations forward only where the content survived.
  ...
  *(3.12)* **A media pair is compared by SAMPLE SEQUENCE, not by bytes**, and the exception is not a convenience — without it the check is structurally incapable of answering the question history §12.37 asks it. A record's identity is its artifact's blake3, so a track re-framed under a changed pinned form (history §12.20 item 1) has a different id **by construction**; comparing whole-artifact identity therefore reports *diverged* for a supersession that is in fact exact. A gate that is always red is worse than no gate, because it gets ignored. What survives a reframe is the **sequence of sample sizes in decode order**: re-enveloping moves every sample's file offset and changes no sample's size, while a re-encode, a dropped or reordered sample, or a differently-framed payload all break it. The sequence is read by the corpus's own engine-free sample-table reader — the same oracle the `framing:` stamp is checked against (§7.1) — so the comparison does not share an implementation with the muxer that produced either side. The four verdicts keep their meanings, with *contained* now naming a sequence B extends. **Which tracks pair is decided by containment lineage, never by position**: where B's origin names `corpus://<A>?stream_id=<n>`, B's single track is compared against A's track `n`, because a leaf's own track 0 may be its container's track 1 and pairing by index would silently compare the wrong two. Absent lineage, tracks pair by index and a count mismatch is reported rather than reconciled.
...
#### 12.9.1 The derived hash index *(v20)*
...
- **Consumers**: the same-document health signals — the duplicate-capture join on `html-stampfree@1` and the grown-export screen on the prefix ladder. The motivating measurement: the byte-reading predecessor signal on a 25k-record / 157 GB corpus read 19.75 GB across 251 same-filename groups and held one group's 9 GB in memory simultaneously — the scan died on the OOM killer, and the inert-since-3.5 content-dedup fold meant no signal covered the ground at all. Index-joined, both signals cost zero artifact reads in the steady state. External checksum verification (`sha256` interop) and any future similarity search (§7.7 fingerprints) read the same rows.
...
#### 12.9.2 The location index *(21)*

The attached-location route's derived map — blake3 → `(location, relpath, size, mtime)`, *(24)* plus row provenance and *(25)* the advisory `mime_claim` where a manifest supplied one — in the §12.9.1 mold: untracked, never authoritative, reference shape a single SQLite file, `cache/locations.db`. It differs from the member index in one load-bearing way: the member index is rebuilt-on-start from records (§12.15) because every row derives from a members block, but an attached file may have **no record yet** — pre-promotion, this index is the only memory of its hash, and recomputing a row means re-streaming the file (a 100 GB mirror makes that hours, not milliseconds). So it persists, exactly as `hashes.db` persists: recomputation needs *bytes*, which is §12.9.1's test for earning persistence — and the "not a general side-store" rule stands, because these rows fail exactly the criterion that keeps other maps record-derived. Wiping it is always safe (nothing normative depends on it — §2); the cost is one re-attest per location. Rows go stale by `size`/`mtime` drift (§12.1.1) and refresh only by re-attest, never silently at read time. *(24)* Rows record their **provenance** — `computed` or `presented` (§12.1.1) — and a presented import is stamped with the manifest **generation** it came from, so re-attest on a presenting location skips generations already ingested: incremental by construction.
...
- **URI index persistence.** The URI → `id` lookup (`records.build_uri_index`) is rebuilt-on-start from the records — the settled default (an in-memory query engine, not a data store). A persistent side-file is a deferred perf optimization, not an open design question. *(v20: the hash index (§12.9.1) now establishes the pattern such a side-file would follow — untracked, regenerable, never authoritative — but records-derived indexes stay rebuilt-on-start until measurement says otherwise.)*
...
### 12.16–12.37 Migration history

This range is non-normative migration narrative, one entry per amendment or implementation increment from 2.0 through 3.12 — the field inventories, the tooling order, the executed sweeps, the open questions raised and resolved. It has moved to [`corpus-history.md`](corpus-history.md), which retains the original §12.x numbers so every cross-reference in this document (and elsewhere in the system) still resolves. The changelog table at the top of this document (§1) is the index; §1.2 and §1's other rationale pointers read `(history §12.x)`.
...
| **Transport** | The media-type-shaped container of a file. Also the name for the members-block row's bytes-level hash field (`transport:`) and the origin-block `source_transport` *(v20: the frontmatter field of this name renamed to `hash:` — §4.2.1)*. |
| **Content** | What a transport carries. Decomposes into segments. |
| **Segment** | An addressable unit of content. Carries an address (optional since 3.8 — absence names the whole transport, §4.3.2.2) and at most one atomic classification; the structural and placement kinds carry no atom at all. |
...
| **Derived envelope** | *(3.7)* A form section's envelope — the min–max span of its children's addresses — computed, never stored; the `address` field a section header once carried for it is retired (§4.3.2.1). With it, the whole-record section stops being spellable: where one form governs everything, a single span's derived envelope simply covers the whole content zone. |
...
| **Proxy** | A record's universal role from birth (3.1, §4.1): the attested, consumable stand-in for its artifact — complete without any stored rendering. |
| **Terminal contract** | *(3.3)* A member of the form domain prescribing the **absence** of a stored rendering — `form/passthrough` (the identity contract, named) or `form/manifest` (the members are the content). Governs a record exactly as a rendering contract does; a terminal record never gates by default and is reported `terminal`, not `proxy` (§7.8). |
| **`rendered`** | The grandfathered state (history §12.19/§12.23): a record carrying a stored rendering under no named form contract, tolerated pending its next pass through normalize — never a steady-state target, and never retroactively condemned by a later class-grain form declaration (§7.8's governing-contract precedence guard). |
| **Formed** | Derived state predicate (3.1): a form section governs the record's stored content zone — a stored rendering under a named contract (§4.1). |
| **Authored / Vouch** | *(retired 3.2, and again in full at 3.5)* The 3.1 predicate — frontmatter title/description non-empty — dissolved into the form layer at 3.2; 3.5 then retired the interpretive editorial fields outright (there is no vouch, at any scope, §4.1). "The authored layer" survives only as an ownership name (what the normalize pass writes, §4.4.7). |
| **Derived editorial fields** | *(3.2; the ladder narrowed to two rungs in 3.5)* A record's display title/description, computed — never stored — from role-marked fields by precedence **artifact → origin** (latest block wins within a layer; an all-empty chain resolves to an honest empty value; §4.2.3). |
| **Role-marked field** | *(3.2)* An `extended_fields` declaration carrying `role: title` or `role: description` — an editorial candidate for the derived title/description (§4.2.3). Declared only on mime schemas and origin overlays; a form contract marks no editorial role — that rung retired in 3.5. |
...
| **Whole-address admissibility** | *(3.10)* A mime schema's `whole_address` declaration (§7.1) — `admissible`, `forbidden`, or `single_unit_only` — stating whether an address-less segment (naming the whole transport) is legal for the medium; the third value was forced by measurement (an `image/gif` may be a single frame or an animation, decidable only from the bytes). |
| **Cut strategy** | *(3.11)* A mime schema's declared `cut_strategy` default (origin-overridable) for deriving a time-addressed stream's segment boundaries. The promoted leaf carries a **`cutting:`** stamp naming the versioned strategy, its parameters, and the resulting cut count — the self-check a disagreeing consumer compares against rather than re-deriving (§7.1). |
| **`framing:` stamp** | *(3.12)* A promoted media-stream leaf's attestation of the muxer, its version, and a sample count (§7.1) — the self-check that admits a muxing engine into the identity path on measurement (byte-identical `-c copy -fflags +bitexact` output) rather than on faith. |
| **Transport hash** | The bytes-level hash of a file or member. Encoded as `<algo>:<hex>`; carried by members-block `transport` rows *(v20: the frontmatter home for auxiliary byte hashes is the `hash:` field)*. |
| **Hash field** | `hash:` — frontmatter home for auxiliary identity hashes *(v20; succeeds `transport:`)*. Tag classifies each value (§7.6): bare algorithm id = byte-stable digest; `<procedure>@<version>` = canonicalized identity, flush-only. |
| **Canonical hash** | A procedure-versioned canonicalized identity, `<procedure>@<version>:<hex>`, resident in the derived hash index and flushable into `hash:` *(v20: re-earned residency under recipe versioning — §4.2.1, §7.9; the 3.5-retired `canonical:` field stays retired, and content-canonical remains barred)*. |
| **Perceptual hash** | An atom-canonical content fingerprint — procedure-versioned, resident in the derived hash index *(v20: the record/segment `perceptual:` fields are retired — §4.2.1, §7.7)*. |
