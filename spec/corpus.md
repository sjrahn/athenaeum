...
**Route unification** *(v32)*. A promoted media-stream leaf's identity bytes are the raw payload (§1.2, §2) — no timescale, no sample tables of their own — so every op on the leaf resolves **through its container**: `corpus://<leaf>?<ops>` and `corpus://<container>?stream_id=<n>&<ops>` are **the same resolution** — one route, one derivation, one cache entry — because the two URIs name the same bytes by the identity equation. Timeline ops (`frame=`, `time=`, `time_range=`, `scenes=`) and `transcribe` read the container's tables through that route and run against the derived playable surface, in the same timeline the container reports; nothing about *addresses* changes — a leaf's `time_range=` axis is the container's own timeline either way. A resolver SHOULD **disclose the base form** when resolving a leaf (print `≡ corpus://<container>?stream_id=<n>` alongside the result), so the equivalence is visible rather than merely true.
