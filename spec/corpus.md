...
3. **Faithfulness.** A record body is a faithful, lossless rendering of the transport's content. Normalization may resolve ambiguity (encoding, broken layout, OCR for scans) but never adds information not present in the source. Descriptive content (a summary of what an image shows, a paraphrase of what was said) is lossy by definition and lives on the addressing segment's or section's `description:` — **not** in a segment body. *(3.4: previously bifurcated between a matching member row's own `description:` field and the addressing segment/section; the per-member field retired, so the addressing segment or section is now the one home — §4.3.1.4.)* Re-segmentation is structural, never editorial. *(3.0)* The same principle governs structure: a **structural segment exists only where the source carries the mark** (§4.3.2.3, the byte-mark rule). A grouping the bytes do not declare — a conversation's per-day break: whose midnight? — is a *read-time rendering* with explicit parameters, never a stored block.
...
A member's **narration** has no home here at all. An asset worth describing is described where the record represents it — on the section or segment that places it (§4.3.2.2) — or on the member's own record once promoted (§8.1). *(3.4 retires the per-asset `description:` field; §12.26 carries the migration.)*
...
Every image, audio, and video segment's address must appear in some member's address — **except resolver-materializable addresses**. When the resolver can materialize the segment's address on demand, no member row is required — the functional URI (§6) produces the bytes, and any description rides the addressing segment's (or section's) own `description:`. Two cases qualify:
...
| Field | Description |
|---|---|
| `description` | The segment's description — what this segment IS at its address and under its overlay. *(3.4: the only home for a description of the thing at this address, whole asset or sub-region alike; the retired per-asset embed block's `description` merged into it, §4.3.1.4.)* Normalizer-written. |
...
- **`image`** — a static image at the addressed region. Body-empty positioning marker; description on this segment's own `description:` *(3.4: uniformly, whether or not a member row exists at the address)*.
- **`audio`** — an audio range. Body-empty positioning marker; description on the segment's own `description:`. Transcripts live as separate `text` segments at the same address.
- **`video`** — a video stream over the addressed time range. Body-empty positioning marker; description on the segment's own `description:`. Captions and scene transcripts live as separate `text` segments at the same address.

###### Faithfulness

The segment body MUST be a faithful, lossless rendering of the addressed content. Descriptive content (a summary of what an image shows, a paraphrase of what was said, or what a live/computed region contains) is **lossy** by definition and belongs on the **addressing segment's or section's own `description:`** — NOT in a segment body. Re-segmentation is structural; content within remains faithful. *(3.4: the header is now the only home. The retired per-asset embed block carried a `description` for the whole asset, which split an asset's narration from the place the record represents it — and put authored prose inside what is now a wholly attested roster, §4.3.1.4. A member's narration rides the block that places it; an asset that deserves more than that is promoted to its own record, §8.1.)*
...
- **Normalize owns the authored layer** — the stored content zone (total replacement per pass: shaper or agent, same discipline the 2.x body-draft contract stated), segment/section `description:`s, the form span's editorial header fields (§4.2.3), asserted faithfulness issues. A re-normalize refreshes it; it never touches attested facts except to *read* them.
...
- Writes asset descriptions on the section or segment that **places** the asset, via `description:` (lossy interpretation — never in a faithful segment body). *(3.4: the members block is wholly attested and off-limits to this pass — it carries no description and no `alt` to fill, §4.3.1.4.)*
...
while id=$(corpus drain --by "$SESSION"); do
    ...
    # ...the agent normalizes $id in-session: title, description, segment/section
...
**Where the narration went.** An asset's description now rides the section or segment that **places** it, or the member's own record once promoted. This is the better home independently of everything above: it puts the narration next to the representation it describes, and it puts it in the **authored layer**, which re-attestation does not touch. The population that had to move: ~12,390 public and ~3,247 private descriptions had an exact-address block to land on; **23 public and 161 private did not** — 104 config members of a diagnostics zip and 57 email attachments, all on records with no content zone at all, where `form/manifest` forbids one. Those are deleted rather than given a fabricated home, and enumerated in the migration manifest so promoting any of them stays available. The 150 vCard display names were never authored — mechanical `FN` reads — and are simply derived now.

**The migration is lazy, and it is re-attestation.** The reader accepts both forms; only the new one is written. Conversion is not a text transform — an HTML `el=` member never stored `bytes`, so the new row needs the artifact — which means conversion *is* an ordinary re-attestation, and no bespoke migration verb is required. The one thing that must not be lazy is the ordering: because record serialization happens on every write path, a naive rollout would emit a members block and drop the pending descriptions on whichever write came first. The write path therefore **refuses** to serialize a roster while un-re-homed legacy descriptions are present. That refusal is the whole safety property; it is the same shape of defect as §12.25's — a correct operation performed before a cheap check — and it is guarded here rather than discovered later.
