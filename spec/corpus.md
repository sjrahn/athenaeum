...
|---|---|---|
| `address` | Span-scope only | The span's **envelope** — the min–max span of the child segment addresses (`pages=2-6`, `turn=1-74`) — derived from the children, never wider than the content held. Omitted only on a **whole-record** form section (one section spanning the entire content zone); required whenever the record carries more than one section, which *(3.5)* is the ordinary case. |
...
***(3.5) There are no universal section-header fields.*** `entry:`, `title:`, and `description:` are retired: `address` is the span's own definition rather than a field about it, and **every other field a section carries is one its form declares.** A form that declares nothing — `form/index` is the measuring case — takes a **bare opener**, and the bare opener is complete: `<!--section index-->` says *this span takes the index form and renders under its contract*, which is the entire job of the block. The retirement is structural, not stylistic. A field present on every section regardless of form is a field no contract owns and no check binds, so it is where accretion lands — first a TOC label, then a title, then a paragraph of interpretation — and each addition looked reasonable against the one before it. Removing the universal slot removes the landing site.
...
**Same-region stacking is the whole answer to lossy content** *(3.5)*. A marker and a faithful extraction of the same region **coexist**, always: they are different representations with different opener-ids at one address, exactly as the identity rule already permits, and neither displaces the other. A photograph is inherently lossy — the marker stands for it permanently — while the text printed *in* that photograph extracts losslessly into a co-addressed `text/ocr` segment. The marker never becomes redundant, because the pixels are never fully reduced to the words; the transcription never needs permission, because it is faithful. This retires the question of whether a full-region transcription "replaces" its marker: it does not, and the question only ever arose because the marker used to carry narration that the transcription would have orphaned.
