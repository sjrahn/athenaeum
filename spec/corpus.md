...
**Timeline inheritance** *(3.11)*. A **timeline op** (`frame=`, `time=`, `time_range=`, `scenes=`) addressed at a promoted **video** stream leaf resolves through the leaf's containment lineage, against the container, with the leaf's own `stream_id=` composed in. This is the case §1.2 already states for chapter marks — *"tracks inherit the container's chapter marks at read time through lineage — never copied, because the track's own bytes do not carry them"* — applied to the timeline the marks sit on, and the reason is identical: **a bare Annex-B video stream carries no timing at all.** No timescale, no duration; a decoder imputes a frame rate from in-band parameter-set timing where present and from a default where not.
