...
*(21)* An **attached location** (§12.1.1) is the third route class: an operator-managed tree whose files stay in place under their own names, resolvable by bare id through a derived location index (§12.9.2) once an attest pass has hashed them. Like the container route, the map is derived and the record never stores its residence; unlike a container, the tree is mutable — so the route serves bytes only while the index's staleness pins hold (§12.1.1), and identity is re-verified in full at every promotion.
