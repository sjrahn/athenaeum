...
A member's **narration** has no home here at all. Where a record narrates an asset it does so where it represents it — on the section or segment that places it (§4.3.2.2), or on the member's own record once promoted (§8.1) — and it narrates only what it has reason to. The roster carries none of it. *(3.4 retires the per-asset `description:` field outright; prose stored in it is dropped with the block, not migrated. §12.26.)*
