...
| **reading** | The mechanical textual reading of an already-addressed place: the embedded text layer (`page=<N>&text`), the markup's own text (`text`, `el=<N>&text`), the derived body (`body`), the attested roster (`members`), the document's own outline (`outline`). Nothing is judged; the text is in the bytes. | no — the segment *is* the reading | yes — at most one, ending the anchor |
...
| **instrument** | A measurement or proposal *about* the artifact, for the normalizer or the querier: `probe`, `page=<N>&words`, `page=<N>&geometry`, `scene`, `scenes=`, `corpus bands`, a contact sheet. Its output is a tool's reading of the artifact — bboxes, chrome flags, shape hints, cut proposals — and citing it cites the tool. | no | **no** |
...
- `sidecar:` *(v41, owner ruling)* — a **container-member sidecar** declaration: for a producer whose export pairs each primary member with a companion metadata member beside it in the same container (osxphotos: `<member>.json`; proton-mail-export: `<stem>.metadata.json`), declares the pairing and what is lifted where. The sidecar member **stays in the container** — a roster row, addressable by `?path=`, resolvable, citable — but it is **consumed**, never a record of its own: `corpus promote` refuses a consumed-sidecar address, naming the primary member instead, and lint reports any record whose sole lineage is a consumed-sidecar address (`sidecar-member-promoted`, error). The engine that reads this key is **producer- and format-agnostic**: its whole vocabulary is a pairing template, a parsed sidecar tree walked by dotted path, a lift map with two generic transforms, reference templates resolved against the container roster, a prefix, and a subtype — nothing in `promote` / `period-split` / `reattest` / `lint` knows what any field means, and a second producer joins by declaration alone. Keys:
  - `pairing:` — `template:` is the member→sidecar path convention, with `{member}` the member's full roster path and `{stem}` that path minus its final extension (`"{member}.json"`; proton `"{stem}.metadata.json"`). **Present-only**: a member whose template expansion names no roster member has no sidecar — nothing is ever invented (an `_edited` render is never handed its original's sidecar). An optional `export_level:` list names export-level metadata members that pair with no primary at all (proton's `labels.json`) — never a member for bucketing, disclosed by the split (§12.3.14). This is the single declaration `corpus period-split`'s date-axis pairing also reads; the v36 code registry is retired.
