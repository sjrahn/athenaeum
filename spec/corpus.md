...
**A container-member sidecar is not a row** *(v45, owner ruling)*. Where the record's own origin overlay declares a `sidecar:` (§7.2), every entry the declaration's template pairs to a present primary is metadata **of that frame**, not an asset of the capture in its own right: attestation omits it from the roster, so it has no member route (§12.9), no promotion (§8.1), and no address of its own (`?path=<sidecar>` is refused, naming the frame). Its bytes stay inside the container — they are part of the container's identity, and the archive's physical facts (`member_count`) still count it — and it is read only **through its frame**: the `sidecar` reading (`?path=<frame>&sidecar`, §6.2) serves it verbatim, and the frame's `members` descriptor carries its projection. An entry whose would-be frame is absent from this container pairs with nothing and stays an ordinary member. A roster attested before v45 still lists its paired sidecars until re-attested (lint `sidecar-member-rostered`, error).
...
- **`single_unit_only`** — a medium that *may* be a sequence, where only the bytes decide. Absence is admissible **iff** the attested count named by `whole_address_count` equals 1. *(`image/gif`, `image/webp`, `image/avif`, `image/heif`, `image/tiff` — `frame_count`; a HEIF may carry an image sequence, which is why the still formats and the motion formats cannot be split by MIME family; a TIFF may carry pages, counted on its top-level IFD chain.)*
...
An `issue` context block (`<!--context issue/<id>-->`) is a typed **fidelity attestation**: a standing statement of where the record's or a segment's representation falls short of its bytes. In the record an issue is an attestation, never a ticket — the namespace name predates the tracker split (Part I §2.3) and is kept for stability. Its universal overlay (`context/issue/issue.yaml`) declares:
...
- `applies_at` — the block scope(s) the id may legitimately be raised at, a subset of `[record, segment]`. `record` admits the id with no `address:` field (a whole-artifact problem); `segment` admits it pinned to one (§4.3.3.1). The universal overlay declares both; a per-id overlay narrows to whichever scope its own semantics actually admit — a whole-capture failure like `bot-block` declares `[record]` only, since there is no single segment to pin it to.

Per-id overlays (`context/issue/<id>`) extend with id-specific fields. The `severity` value set is corpus-local (schema-declared); external tooling should treat unknown values gracefully rather than assuming a fixed vocabulary. Issues come from two sources — the **deterministic pipeline** (mechanical detections at capture/ingest: bot blocks, corrupt encoding, missing inputs) and the **normalizer** (faithfulness problems found while rendering) — each recorded in `detector` and, where engine-owned, `provenance: auto`. Only the pass that had the artifact open emits one; a finding made after the fact routes elsewhere (below).

**An issue is lifecycle-free** *(v42; the `resolution` field is retired)*. A block is never edited toward a resolved state: its presence is always a current-truth statement of the latest emitting pass, and it is **dropped, not marked fixed**, when the fact it attests stops being true — a `provenance: auto` block by its detector's strip-and-regenerate on re-run (§4.3.3.1), an asserted block by the re-normalize that owns the authored layer (§8.1). A *fixable* finding is report material and never becomes a block: it is filed on the instance tracker (Part I §2.3) — the layer that owns priority, assignment, and closure — and its fix lands in the record, where the touch chain proves it. Git history holds what the record used to attest; the tracker holds the lifecycle; the record holds only what is true now. This is the lifecycle discipline the `sweep` namespace has always had (§4.3.3.6): two attestation namespaces, negative and positive, neither carrying state that can go stale. Where a fidelity defect bears on *epistemic* content — evidence a ledger fact or interpretation cites — it additionally warrants a ledger assessment citing the record (Part III §7), retired when the fix lands; the tracker issue is its operational twin, never its substitute.

**An issue is a typed code at an address, and carries no prose.** Its whole payload is `(<namespace>/<id>[/<subtype>], address, severity, detector)` plus whatever **structured** id-specific fields its overlay declares — a count, a URL, a signature. A sentence explaining the problem is the record narrating itself, it cannot be checked against anything, and it is the exact seam through which summary re-enters a zone that exists to be a side-channel. The type carries the meaning — `partial-content/paywall` at `el=12` needs no gloss — and this is the same obligation §4.3.2.2 puts on atom overlays: an issue id too vague to be self-explanatory is under-specified, and the fix is a **better id**, never a sentence. Where a fidelity problem genuinely warrants explanation at length, it is an interpretation in the ledger, which can cite the record and be revised.

**An issue is not a derivation.** A finding that a read-time pass could recompute from the record and its schemas — *this record's derived title is generic*, *this record carries no form* — is a **health signal**, computed on demand, never persisted into records. Storing it makes the record assert a verdict about itself that goes stale the moment either side changes. The test: if a sweep over the corpus could produce the finding from what is already there, it belongs to health; if only the pass that had the artifact open could know it, it belongs here. **A post-hoc reviewer finding is neither**: a defect of the authored layer found by *reading* the record — a rendering flaw, a mis-declared field — is recomputable by anyone holding the record and its bytes, so it is never persisted as a block; it is a work item for the tracker (Part I §2.3), and its fix is a re-normalize whose output no longer contains the defect.
...
- **Ingest / promote own the attested layer** — the artifact block's fields, manifest members, structural byte-marks, sidecar-lifted origin fields (a capture-time lift persists as stamped, §7.1; a container-member lift is regenerated from the container under its declaration, §7.2), origin stamping. A **re-attest** (§8.3) strips and regenerates them from the current schemas; it never touches the authored layer.
...
- `<params>` — `&`-separated key/value pairs and flag-style keys. Order is significant — parameters compose left-to-right, each operating on the previous step's output. Each `&`-delimited chunk splits into key/value on its **first** `=` only; a value carrying further `=` characters keeps them as raw value bytes — unpadded vs. padded base64, a transcribed heading that itself contains `=`. A param **value** percent-encodes the query-reserved characters `%`/`&`/`#` as `%25`/`%26`/`%23`; the parser decodes, and canonicalization re-encodes. `=` itself is deliberately not in that escape set — the first-`=`-wins split already makes it safe unescaped. Values may therefore carry any character — archive member names are producer-controlled (`?path=…D%26D 5e….json` addresses a member literally named `…D&D 5e….json`).
...
| Param | Class | Input type | Output type | Description |
|---|---|---|---|---|
| `page=<N>` | address | PDF | image | Select page N (1-indexed). On its own it renders the page as an image; an image op or `bbox=` after it auto-renders first. |
| `page=<N>&render` | view | PDF page | image | Render the selected page as an image (the explicit form of a terminal `page=<N>`). |
| `page=<N>&text` | reading | PDF page | text | The page's embedded text layer, verbatim — not an OCR of the raster; empty when the page carries no text layer. |
| `page=<N>&words` | instrument | PDF page | json | The page's text-layer words, each with a `bbox` (`[0.0, 1.0]` page fractions, origin top-left). |
| `page=<N>&probe` | instrument | PDF page | json | Per-page structural signals: dimensions, rotation, text/image-coverage stats, an invisible-text flag, and an advisory shape hint. |
| `page=<N>&geometry` | instrument | PDF page | json | The page's text-layer line geometry: assembled visual lines (position, extent, text), chrome flags, heading candidates, and the non-chrome content band — exact coordinates for boundary-setting, read from the one place they are exact. |
| `probe` | instrument | PDF | json | Whole-document structural probe: per-page table, `/Info`, outline presence, and a shape summary. |
| `outline` | reading | PDF | json | The PDF outline / TOC tree (nested `{title, page, children}`). |
| `time_range=<s>-<e>[,<s>-<e>…]` | address | media container / stream | media slice | A cut of the **composition** (on a container: the default member of each kind, muxed — the muxing contract, below) or of one stream (after `stream_id=`). A comma-delimited ordered list concatenates cuts in listed order. |
| `stream_id=<id>` | address | multi-stream media | stream-isolated | Select a specific stream. **Terminal**, it materializes the member's identity bytes — the raw payload (§2), byte-identical to resolving the promoted leaf's bare id. With further ops it selects the stream for the derived-rendering chain (cuts, `format=`, `transcribe`), which runs against the derived playable surface. |
| `frame=<N>` \| `frame=<A>-<B>` | address | animated image / video | image / — | **Polymorphic** (see below). On an **animated image** (GIF, animated WebP/AVIF): a **1-based ordinal** selecting one frame of the sequence, bounds-checked against the attested `frame_count`; leads its chain, so `frame=3&bbox=…` names a region *of frame 3*. The inclusive span form `frame=<A>-<B>` is a **stored-address assertion** (§4.3.2.2 — a rendering that holds across every frame in the span); it names no single byte surface and does not materialize. On a **video**: a timecode (seconds or `HH:MM:SS`) rendering the frame at that timestamp to an image. A stored image-side value that breaks the ordinal grammar, exceeds the attested count, or trails a region param is a lint ERROR (`address-frame-invalid`). |
| `bbox=<x>,<y>,<w>,<h>` | address | image / spreadsheet | image / cell-range | Crop a relative region (image: floats in `[0.0, 1.0]`, origin top-left) or narrow a worksheet (spreadsheet: an A1 range, e.g. `bbox=B2:G30`). Polymorphic — see below. The 3rd and 4th values are a **width and a height, not a second corner**: `x+w` and `y+h` must each be ≤ 1. A stored region-op value that breaks this grammar is a lint ERROR (`address-region-invalid`). |
| `crop=<x>,<y>,<w>,<h>` | address | image / spreadsheet | image / cell-range | Alias for `bbox` (inherits its polymorphism). |
| `mark=<x>,<y>,<w>,<h>[;…]` | view | image | image | Outline the region(s) on the **whole** image (does not crop) — the inspection dual of `crop`, showing where a region sits in context. Relative floats in `[0.0, 1.0]`; `;`-separated for multiple regions. |
| `cover=<x>,<y>,<w>,<h>[;…]` | address | image | image | Paint the region(s) **out**, filled with the background colour sampled from the ring just outside each box (never assumed white). The chrome remover, for captures that baked a viewer's own controls into the artifact. Does not crop — chain `crop=`/`bbox=` after it when a crop is also wanted. Lossy by nature (it deletes pixels), and disclosed by riding the address: the same surface resolves without the op to show what was removed. Same grammar as `mark=`. |
| `resize=<W>x<H>` | view | image | image | Resize to absolute pixel dimensions (forces both, may distort or enlarge). |
| `fit=<W>x<H>` \| `fit=<preset>` | view | image | image | Downscale to fit within a bounding box, aspect-preserving; reduce-only (never enlarges). A `<preset>` names an implementation-defined budget. |
| `rotate=<90\|180\|270>` | address | image | image | Rotate clockwise by a quarter turn (lossless; 90/270 swap width and height). |
| `auto_orient` | address | image | image | Apply the image's EXIF orientation tag so a sideways/flipped capture displays upright. No-op when absent. |
| `autocontrast` | view | image | image | Stretch the per-channel histogram to full range (legibility for faint scans). |
| `contrast=<factor>` | view | image | image | Scale contrast by a float factor (`1.0` unchanged). |
| `grayscale` | view | image | image | Convert to single-channel grayscale. |
| `dpi=<N>` | view | (render config) | (config) | Rasterization DPI for `page=<N>`. Position-independent. Default 200. |
| `body` | reading | any | markdown | The record's **derived body** — the faithful mechanical rendering: DOM→markdown under the overlay's capture-time chrome config (HTML), spine text (EPUB), reply-text trim (eml), verbatim passthrough (JSON, plain text), per-page content-band text with scanned-page and outline-entry markers (PDF). A pure function of (artifact × schemas × op version); what `corpus body` prints for a formless record. |
| `members` | reading | any transport with embedded assets | json | Member enumeration over every address axis (`el`/`spine`/`path`/`msg`/`part`/`card`/`stream_id`/…) — the roster **with its full descriptors**, derived. Beyond the stored row's four keys (§4.3.1.4) it carries every mechanically-readable per-member fact: pixel dimensions, verbatim `alt`, member filename, an email member's `from`/`subject`/`date`, a vCard's display name — and *(v45)*, for a frame whose container declares a `sidecar:` (§7.2), its sidecar's projection: exactly the prefixed fields promotion would lift, one projection for both. The surface a normalize pass consults to see what an artifact carries — never the block, which by design says less. Version-labeled (`members@2` since v45's roster change). |
| `sidecar` | reading | a member of a container declaring `sidecar:` (§7.2) | json | *(v45)* The frame's paired metadata sidecar, its bytes **verbatim** — `?path=<frame>&sidecar`, terminal, one frame; a promoted frame takes it bare (`corpus://<frame>?sidecar`) through route unification. Pairing is physical (the container's own entries under the declaration's template); a frame with no sidecar beside it is a hard error, never a substitute. The sidecar has no address of its own (§4.3.1.4) — this is the only way to it, and the form an evidence anchor quoting it takes. Version-labeled (`sidecar@1`). |
| `annotated` | view | HTML | html | *(v35)* The **annotated view** — the artifact's original bytes with every element's document-order ordinal (§6.1.1) spliced into its own start tag as a `data-el="<N>"` attribute, span-surgically (no re-parse, no re-serialization; every source byte kept in order). The authoring surface, and the **default delivery**: bare `corpus://<hash>` on a markup artifact resolves here, so a reader cannot cite by guesswork — the correct coordinates are forced into view. Computed under the record's attested parse, version-labeled (§6.4), cache-only, provable by re-derivation. **Terminal only**, like `raw` — a delivery view, not a working step: ops run against the raw parse, never against stamped bytes. The explicit spelling exists for disclosure and symmetry; it and the bare route are one resolution, one cache entry. |
| `raw` | view | any | bytes | *(v35)* The stored artifact bytes, exactly — for a markup artifact, the escape from the annotated default (rarely needed: byte-level verification, re-export, external diffing); for every other type the bare route already is this. `blake3(?raw) == <hash>` is the identity equation's markup spelling. Terminal only — it composes with nothing. |
| `text` | reading | HTML | text | *(v43)* The markup's **text reduction**: every text node in document order, block-level elements and `<br>` as line breaks, table cells tab-separated, `<head>`/`<script>`/`<style>`/`<template>` dropped, whitespace normalized — the artifact's own text and nothing else: no chrome judgment, no quote trimming, never a rendering. Mechanical and unversioned like `page=<N>&text` (the parser is the one §6.1.1 pins). The honest citable surface of a **message-borne HTML body** (`msg=<N>&part=<M>&text`), where `block=1`'s reply-only trim is the wrong surface for forwarded content; on a `segments`-surface capture (§7.1) the whole-document reduction is exactly the presentation soup that surface exists to refuse — cite the element form there. |
| `el=<N>&text` | reading | HTML element | text | *(v43)* The element's own subtree, reduced exactly as `text` — the span-precise form, and the explicit textual materialization of a text element (bare `el=<N>` names the element, whose content is its region, §12.11). |
| `header=<name>` | address | MIME message | text | *(v43)* One header of the message, RFC 2047-decoded and folded to a single line (a repeated header yields every instance in file order, newline-separated; the name is case-insensitive) — the envelope's own facts (`subject`, `from`, `to`, `cc`, `date`, `message-id`, `in-reply-to`, `references`) as a span-precise, verifiable surface without promoting the message: `msg=<N>&header=subject` on the mailbox itself. A header the message does not carry is an error. Engine-pinned `eml-header@1` — the `prop=` shape on the mail axis. |
| `transcribe` | engine | audio / audio stream | json/text | Speech-to-text over the addressed audio, with timestamps and speaker-turn indexes where determinable. **Version-labeled** (§6.4): the result carries engine + model version. |
| `scene` | instrument | promoted media stream leaf | json | The leaf's **cut list** under its stamped `cutting:` strategy (§7.1): the resolved strategy, every span with its `time_range=` address and length, and the degeneracy signals (`no-boundaries`, `over-segmented`). The introspection surface an authoring pass reads before deciding anything — `probe`'s role for a PDF page, on a timeline. Runs the **stamped** strategy, never a fresh resolution, so this op can never be the route by which a record is silently re-cut; an unstamped leaf answers `unresolved` rather than failing, because that is a legitimate state (§7.1). |
| `turn=<N>` | address | turn-structured record | json/text | The verbatim N-th unit of the record's declared unit array (1-indexed), located by the origin overlay's form mapping (§7.2) — for a chat transcript, the complete message object: reactions, edit history, attachment declarations, platform ids, one hop away from the envelope segments. |
| `turn=<N>&att=<M>` | address | turn-structured record | bytes | The M-th attachment declared by unit N, materialized through **lineage-chained resolution** (below). |
| `cut=precise\|copy` | view | (cut config) | (config) | Cut semantics for `time_range=` (the muxing contract): `precise` (default — frame-accurate, re-encodes) or `copy` (keyframe-snapped stream copy, disclosed). Position-independent, like `dpi=`. |
| `format=<token>` | view | media / image | converted rendering | Output-format conversion, composing **after** selection and cutting (the muxing contract): `time_range=12:04-12:09&format=gif`. Changes encoding only, never the addressed content. |
| `scenes=<threshold>` | instrument | video / stream | boundary proposals | Scene-cut boundary proposals over the video timeline — a text listing of cut timestamps at the stated detection threshold, engine-versioned (§6.4). Normalizer support for boundary work: proposals to be verified by the pass, never marks. |

A parameter applied to an incompatible working type is a hard error.

**Op classes** *(v43, owner ruling)*. Every op — a transformation above, a record-level op, or a bare address axis (§12.11) — belongs to exactly one **class**, declared once per op name (a class is a property of what the op *names*, never of the working kind it runs on: `text` reads a PDF page and an HTML document alike; `bbox` addresses an image and a worksheet alike). The class is the op's **place**: where it may appear.

| Class | What it is | Stored in an address | Cited as an anchor |
|---|---|---|---|
| **address** | Names a place in the bytes: the selectors (`page=`, `el=`, `bbox=`, `frame=`, `time_range=`, `stream_id=`, `row=`/`col=`, `msg=`/`part=`/`path=`/`card=`, `prop=`, `block=`, `turn=`/`att=`, `spine=`, …), plus the frame a region is measured in (`rotate=`, `auto_orient` — lossless quarter-turns; a region authored in the rotated frame carries its frame) and the disclosed region subtraction (`cover=`). | yes — a stored address is made of these only | yes |
| **reading** | The mechanical textual reading of an already-addressed place: the embedded text layer (`page=<N>&text`), the markup's own text (`text`, `el=<N>&text`), the derived body (`body`), the attested roster (`members`), the document's own outline (`outline`), a frame's paired metadata sidecar (`sidecar`, v45). Nothing is judged; the text is in the bytes. | no — the segment *is* the reading | yes — at most one, ending the anchor |
| **view** | A rendering for eyes, or its configuration: `render`, `fit=`, `resize=`, `mark=`, `autocontrast`, `contrast=`, `grayscale`, `format=`, `dpi=`, `cut=`; the delivery forms `annotated` and `raw`. Changes how a place is shown, never which place. | no | **no** — a view op in an anchor is a mis-authored anchor |
| **instrument** | A measurement or proposal *about* the artifact, for the normalizer or the querier: `probe`, `page=<N>&words`, `page=<N>&geometry`, `scene`, `scenes=`, `corpus bands`, a contact sheet (`corpus contact-sheet`, §12.9.3). Its output is a tool's reading of the artifact — bboxes, chrome flags, shape hints, cut proposals — and citing it cites the tool. | no | **no** |
| **engine** | Model output: `transcribe` (and OCR, which is normalize-pass work). Citable only once a pass has **consumed** it into the record with provenance (§6.4, pinning by authorship) — the stored transcript segment is the anchor, never the live derivation. | no | **no** — cite the stored segments |

Two consequences are normative. **A stored address is address-class only** — lint `address-op-class` (error): a view, instrument, engine, or reading op in a stored address bakes a tool's output, or the segment's own rendering, into what must be a coordinate, so the segment claims a place the artifact does not have. **An evidence anchor is address-class ops, optionally ending in one reading** (`ledger.md` §6.2, §13.2): a view, instrument, or engine op in an anchor is a citation defect at the claim's severity — never the honestly-unverifiable path, which exists for surfaces the verifying environment cannot run, not for anchors that should not exist. The class is **declared in the tooling's op registry once per op name, and an unclassified op cannot register** — so the per-mime op list `corpus inspect` prints (every op reachable for the record's media type, its output kind, its engine pin, its class) is complete by construction: what is available, and its place. `ops_for_media_type` exposes the class beside the pin, and the ledger reads both from it.


A PDF `page=<N>` is a **page selector**, not an unconditional render: a per-page op after it (`render`, `text`, `words`, `probe`, `geometry`) reads the *selected page* directly, so `page=<N>&text` returns the page's embedded text layer rather than an OCR of its render. A terminal `page=<N>` (and any image op or `bbox=` after it) renders the page to an image, so an `address: page=<N>` image marker (§4.3.2.2) still resolves to the page bytes. The whole-document ops `probe` and `outline` operate on the PDF itself (no page selected). The drafter itself runs this shape determination now, per page, at draft time (§12.4.1): a born-digital page's text lands pre-extracted at a measured content-band address, a scanned page keeps a bare marker, and the outline's top-level entries draft as structural byte-marks. These introspection ops are therefore how a normalizer, or any reader, **re-derives and audits** every one of those drafted facts — the band a page was cut to, the chrome a band excluded, the shape a page was judged to carry — never how the shape is first discovered.
...
**Member re-chaining** *(generalized to every member axis, v33)*. A member materialized through **any member axis** (§12.11 — `path=` for a kept-whole archive, `msg=` for a mailbox, `part=` for a MIME message, `card=`/`entry=`/`item=` alike) is not necessarily a terminal opaque byte-string: once extracted, the resolver re-detects the member's own mime from its bytes and declared name and, when a further transform follows in the chain, re-enters the working-kind table (§7.1) for that mime — a PDF member takes `page=`/`text` exactly as a top-level PDF record would, a CSV member takes `row=`/`col=` (§12.11), a mailbox's message takes `part=` (`?msg=23&part=3` reaches an attachment through the mbox — the composition the mbox precedent always implied), and so on for every mime a `working_kind:` schema declares. A **terminal** member address (no further transform follows) never promotes to a full working-kind object merely to re-derive one — that would risk a silent, unrequested re-encode of a member no op asked to transform; it decodes only an already-textual member (JSON, plain text) to its text, so a citation prints cleanly rather than as an opaque cache path — *textual* judged by the member's declared type (a MIME part's Content-Type, decoded per its charset), else its name, else *(v43)* its bytes: valid UTF-8 with no NUL byte is plain text (`config/docker.cfg` in a diagnostics archive has no mimetypes entry and no magic, and must not stay an opaque `.bin` for it), while a NUL or a non-UTF-8 byte keeps the member opaque. The judgement is pinned with the member axis's engine (`archive-path@2`, `eml-part@1`); a roster row's attested `media_type` never uses it. A member whose mime supports neither — no declared working kind, not textual — stays the raw `bytes` a member axis serves by default, byte-identical to the address alone.

**Intra-container references** *(v43)*. A carrier in **message-borne HTML** whose source is a `cid:` URI (RFC 2392) does not inline its bytes: it names a sibling MIME part by `Content-ID` — a reference *inside* the container — and the resolver treats it exactly as it treats a member address. `msg=<N>&part=<M>&el=<K>` on an `<img src="cid:…">` materializes the part carrying that Content-ID **through the enclosing message**: the same bytes its embed row attests (the eml drafter's `content_id`, §12.4.1), never a copy spliced into the stored HTML. This is the mail axis's counterpart of the `data:` URI a web capture bundles (§12.3.6): the capturer inlines because a fetched page has no container to refer back into; a message *is* the container, so the reference resolves in place and the stored bytes stay the identity — a producer canonicalization that rewrote message bodies to inline them would put a re-serializing engine into the identity path (the v32 lesson) and re-mint every closed stratum (§12.3.14) for a handful of members. Scope is the enclosing message only — the `part=` step supplies it — so an HTML artifact with no enclosing message and a `cid:` carrier is an unresolvable reference (a hard error naming the missing part), never a quiet blank. **A member's `el=` speaks the current grammar.** §6.1.1's frozen grammars exist for *stored* addresses on unstamped or dotted-stamped records; a container member has no record and no stored addresses, so its `el=` is the ordinal space, always — stamped by the resolver at re-chaining under the pinned parser identity, with no element count to check.
...
- `sidecar:` *(v41, owner ruling)* — a **container-member sidecar** declaration: for a producer whose export pairs each primary member with a companion metadata member beside it in the same container (osxphotos: `<member>.json`; proton-mail-export: `<stem>.metadata.json`), declares the pairing and what is lifted where. The sidecar is **metadata of its frame, never a member** *(v45, owner ruling 2026-09-22 — v41 kept it a roster row, addressable and citable, refused only at promote)*: its bytes stay in the container, but it is not a roster row (§4.3.1.4), has no address of its own (`?path=<sidecar>` is refused, naming the frame), and is never a record — `corpus promote` refuses it, naming the frame, and lint reports any record whose sole lineage is a sidecar address (`sidecar-member-promoted`, error). It is reached only **through its frame**: the `sidecar` reading (`?path=<frame>&sidecar`, §6.2) serves its bytes verbatim, the frame's `members` descriptor carries its projection, and promotion lifts it onto the promoted record. The engine that reads this key is **producer- and format-agnostic**: its whole vocabulary is a pairing template, a parsed sidecar tree walked by dotted path, a lift map with two generic transforms, reference templates resolved against the container roster, a prefix, and a subtype — nothing in `promote` / `period-split` / `reattest` / `lint` knows what any field means, and a second producer joins by declaration alone. Keys:
  - `pairing:` — `template:` is the member→sidecar path convention, with `{member}` the member's full roster path and `{stem}` that path minus its final extension (`"{member}.json"`; proton `"{stem}.metadata.json"`). **Present-only**: a member whose template expansion names no entry of the container has no sidecar — nothing is ever invented (an `_edited` render is never handed its original's sidecar). Pairing is **physical** *(v45)*: it reads the container's own entries (attestation's full enumeration; the archive itself afterwards), never the roster, which omits paired sidecars. An optional `export_level:` list names export-level metadata members that pair with no primary at all (proton's `labels.json`) — never a member for bucketing, disclosed by the split (§12.3.14). This is the single declaration `corpus period-split`'s date-axis pairing also reads; the v36 code registry is retired.
  - `format:` — `json` (the only value today).
  - `prefix:` — the lifted-field prefix (`osx_`), the counterpart of the capture-time `ytdlp_`: the origin opener names the producer, the prefix names the sidecar as the field's source. Required whenever `lift:` or `references:` is non-empty.
  - `lift:` — a map of `<field>: <dotted sidecar path>` (`date_original: date_original`, `place_address: place.address_str`), each landing on the origin block as `<prefix><field>`. Scalars, and lists of scalars, are lifted (`string | integer | number | boolean | string_or_list` — the field-type vocabulary gains `boolean`); a nested object is **never lifted whole** — flatten by declaration or leave it. An entry MAY instead be a mapping with `path:` plus `omit: [<value>, …]` (a list filter — `persons: {path: persons, omit: [_UNKNOWN_]}`), or `flags: [<key>, …]` (collapse a set of boolean sidecar keys into one `string_or_list` of the names of the set ones, in declared order). **Present-only**: a `null`, empty, or `false` value writes nothing.
  - `references:` — sibling assets the sidecar declares (§6.2 lineage-chained references): `<field>: {template: "{stem}_edited.jpeg", when: hasadjustments}` — `template` expands as `pairing.template` does, `when:` is a dotted sidecar path whose truthy value enables the entry (absent → always). Resolved against the **container roster** at promotion and stored as the sibling's member address (`path=<roster path>`) under `<prefix><field>`; present-only, never invented. A stored pointer into a content-addressed parent cannot rot.
  - `subtype:` — the overlay subtype the promoted record's lineage origin block is qualified with (`photo` → `<!--origin osxphotos-export/photo-->`), whose overlay's `extended_fields` declare every lifted name with type and role (`osx_original_filename` `role: title`). Required whenever `lift:` or `references:` is non-empty. The block's `uri:` stays the containment lineage — promote still adds no producer URI (§8.1); only the qualification and the lifted fields are added.

  Lifted fields are **attested facts** on the lineage origin block (§4.4.6): `corpus reattest` strips and regenerates exactly the names the declaration owns — the whole `<prefix>` namespace, since the prefix is what marks a field as the sidecar's (a name a prior declaration lifted and the current one no longer does retires cleanly rather than lingering) — and touches nothing else on the block: an operator's own stamps, outside the prefix, survive. They are non-primary-source metadata: never the artifact block (whose EXIF / dimensions attest from the bytes, §7.1), never the body, never the frontmatter. **Two sources, side by side, never reconciled** *(v45)*: a promoted frame's artifact block attests the frame's own bytes (its EXIF capture time, camera, GPS, dimensions — §7.1) and its lineage origin block carries the sidecar's lift; where the two disagree, both stand as attested — which to believe is ledger judgment, not attestation's. Origin-block fields are not a citation surface; the ledger cites the sidecar **through its frame** — `?path=<frame>&sidecar`, a reading an anchor may end in (§6.2 op classes; `ledger.md` §6.3) — and a quote verifies against its verbatim bytes. The capture-time yt-dlp lift keeps its own contract (§7.1 `sidecar`, §12.3.7): host-agnostic, one-shot, `comments[]` → `ytdlp_comments`, `webpage_url` / `original_url` folding into the origin `uri:` aliases; nothing mechanical writes body content either way — the **transcript** is the `transcribe` derivation op's output (§6.2), consumed at normalize.
- `date_index:` *(v44)* — the **index-file date axis** `corpus period-split` reads (§12.3.14): for a producer whose members carry no per-member sidecar and whose dates live in ONE index file keyed by member path (a Meta export's `stories.json` is `{ig_stories: [{uri, creation_timestamp, …}]}`; its `posts_N.json` a list of posts each with `media: [{uri, creation_timestamp}]`), declares where the entries are — one mapping or a list of them, each `file:` (a glob over member paths naming the index file or files), `entries:` (the dotted path from the index document to its entry array; an array met along the path is traversed, so `media` over a top-level list of posts yields every post's media entries, and an empty path means the document *is* the array), `key:` (the entry field whose value is a member path — matched exactly against the roster, never by suffix). What the axis READS at each entry — the date field — is named at the verb (`--date-from index:<dotted-path>`), exactly as the sidecar axis names its field: the declaration is the producer's structure, the verb is the split's choice. An index member is never a primary: excluded from bucketing and disclosed (`date_index_files`), as `pairing.export_level` is. Resolved through the same id-prefix ladder as `partition:`; a malformed declaration is refused where it is consumed, never skipped.
...
on <!--context issue/<id>[/<subtype>]-->:
  issues += { id: "<id>[/<subtype>]", severity, detector, address?, ...fields }
```
...
- **Automatic OCR of scanned pages.** *(Narrowed, v40 — was "Automatic PDF text / OCR extraction.")* The drafter (§12.4.1) now performs shape determination itself, per page: a **born-digital** page's embedded text layer is extracted at draft time to a measured non-chrome content-band address (`page=<N>&bbox=0,<top>,1,<height>`, the `geometry` op's own coordinates, §6.2), and the PDF's top-level outline entries draft as structural byte-marks — an automatic pass that the prior text described as deferred. What remains deferred is the **scanned** side: a scan's baked-in text layer (where one exists) is a pre-capture OCR pass, never trusted, and turning its pixels into a corpus-provenanced `text/ocr` transcription (with engine + confidence attested — the corpus owns OCR provenance rather than laundering a pre-baked one) stays **normalize-pass** (agent) work over the resolver's introspection ops, never an automatic one.
...
- **File-grain sources split with `corpus period-split`** — the §12.3.13 split's sibling for a directory/zip of member FILES rather than mailbox messages (a photo library: originals + JSON sidecars; any per-file export tree). Each member's date resolves through a declared **date axis** — a paired sidecar's dotted path (e.g. a photo's PhotoInfo JSON `date`); or *(v44)* an **index entry's** dotted path, for a producer whose dates live in one index file keyed by member path rather than beside each member (§7.2 `date_index:` — a Meta export's `stories.json` / `posts_N.json` `creation_timestamp`; its member mtimes are the export-run time, so the mtime axis would be dishonest there); with file mtime as the explicit fallback axis. A sidecar or index date value may be an ISO string **or an epoch-seconds integer** *(v36)*, which is UTC by definition and satisfies the §12.3.14 boundary rule trivially — the boundary and rendered-local rules read a value the same way whichever axis found it. On the index axis, an entry naming no member of the export and a member the index dates two ways are counted and disclosed on every emitted sidecar (`date_index_unmatched`, `date_index_conflicts`); the two-way member buckets undated rather than by guess, and a member with no entry lands undated exactly as a sidecar-less member does. The member↔sidecar **pairing is producer-declared** — resolved through the origin overlay's `sidecar.pairing` (§7.2) on the same `--origin` ladder as the partition schedule *(v36 introduced the pairing as a tooling registry; v41 moves it into the declaration, which `promote` reads too)*; an export-level metadata file that pairs with no member (a `labels.json`, declared as `pairing.export_level`) is **not a member** — it is excluded from bucketing and disclosed in the split's sidecar counts, never allowed to bucket as an undated primary. Members bucket into ONE deterministic zip PER closed month: each month container is a record directly (file-grain months are records; mbox months are promotable members of one container, §12.3.13), plus the rolling current-month container and the standing undated bucket, sidecar-stamped exactly as the mbox flow's outputs are. Two honest divergences from the mbox flow: (1) a generic zip container carries no media-type-level `default_origin` binding to lean on — `application/zip` is universal, and presuming a producer from it would be dishonest — so the producer is named at the verb (`--origin`), and the partition schedule resolves through THAT ladder; (2) closed-month byte-stability additionally depends on the producer preserving both member bytes AND the dates the axis reads — exactly what the `measured` onboarding mode establishes before a byte-stability-claiming declaration ships (the `settled-first-cut` mode sidesteps the dependency entirely by never re-cutting — the gate bullet below).
...
| PDF | work | `/Info` fields; **exposable members** for embedded files / portfolio members (`attachment=<N>`) | `body` (per-page content-band text, scanned-page markers, outline-entry structural marks), `page=` + introspection ops (`text`/`words`/`probe`/`geometry`/`outline`), `attachment=` |
...
- **Issue loading and parse tolerance.** `schemas.load_context_schema(corpus_root, "<ns>/<id>")` layers `context/<ns>/<ns>.yaml` → `context/<ns>/<id>.yaml`. `records.iter_issue_blocks` / `append_issue_block` are shims over `_contexts` filtered to the `issue` namespace, so pipeline detectors, `health.fidelity_attestations`, the §9.2 view, and lint's issue rules share one path. The reader is parse-tolerant: a legacy `<!--issue <id>-->` still loads (as the `issue` namespace) and upgrades to `<!--context issue/<id>-->` on the next write; a retired `resolution:` field (v42) is likewise read tolerantly and shed on the next write (`corpus retire-resolution` sweeps the fleet: `open`/`wontfix` blocks lose the field, `fixed`/`superseded` blocks are dropped outright — the fix already landed; git holds the history).
- **Reference lint.** `context-namespace-unknown` flags a block whose namespace has no `context/<ns>` overlay.
- **Decompose/compile conventions.** The manifest keeps a dedicated `issue <id> sev= detector=` line for the issue namespace (a legacy `res=` is read tolerantly and shed on write — v42) and a generic `context <ns>/<id> k=v…` line for the others (`recordbuild.add_context`). Two conventions keep the working dir hand-editable: record-level manifest facts are authored only on the manifest `record …` line (not duplicated in `meta.yaml`, where an edit would be a silent no-op), and `meta.yaml` renders a multi-line string as a YAML block literal (`|`) so a multi-line value never reads as a truncated stump. An address list in the manifest is bracketed and `|`-separated (`[a|b|…]`), not comma-separated — a single address (e.g. `bbox=x,y,w,h`) already contains commas.
...
**Measure the boundary, don't eyeball it off a render.** A PDF band's edge is a citable fact, not a guess read off a preview image, and two instruments exist precisely so it never has to be one: `corpus bands <record> [--rows N]` (§6.2's audit CLI) renders the page and measures ink rows directly — the pixel-truth read, unaffected by any text-layer defect, though it can only say *where* a row sits, not what it says — and `corpus resolve 'corpus://<hash>?page=<N>&geometry'` reads the PDF's own text layer for the same coordinates plus what each line contains (chrome flags, heading candidates). Boundaries set from either instrument are exact; boundaries estimated off a rendered page were, empirically, the largest single defect class this guidance exists to close — a crop starting below its own footnote marker, a section boundary slicing the heading it was meant to introduce. `corpus bands` auditing an already-written band is the closing half of the same loop `preview --from-segments` closes for images: **zero bands cutting a text row** is the bar.
...
**Re-propagation.** Corpus-side overlay changes re-propagate with `corpus reattest` (attested layer, scoped by `--host`/`--origin`/`--mime`; the records diff is the review surface) and re-normalize sweeps (authored layer, §8.5). Ledger-side rule changes re-propagate with `ath ledger harvest` — records are untouched.
...
|---|---|---|
| element | `el=<N>` · `el=[<a>-<b>]` (sibling range) · list | marked-up / HTML text (a 1-based document-order ordinal over every element, §6.1.1; output determined by the element — an `<img>` renders to an image, a `<video>`/`<audio>` or `<a href="data:…">` attachment carrier materializes to its raw bytes, a text element to its region — `el=<N>&text` for its text, §6.2; a `cid:` carrier in message-borne HTML materializes through the enclosing message, §6.2 intra-container references) |
...
| part | `part=<N>` | MIME message parts (email; 1-indexed addressable parts — every leaf part plus every nested `message/rfc822` as a whole, in depth-first pre-order — as CTE-decoded payload bytes; promotable per §8.1. A terminal `part=<N>` on a textual part decodes to its text (§6.2 member re-chaining), and `part=<N>&text` reduces an HTML part to its text — the citable surface of an HTML-only message) |
| header | `header=<name>` | MIME message headers (email; the named header's RFC 2047-decoded single-line value — the envelope's own facts, citable span-precisely on the mailbox itself, `msg=<N>&header=subject`; a repeated header newline-joins in file order; §6.2, pinned `eml-header@1`) |
...
| **Context block** | `<!--context <ns>/<id>[/<subtype>]-->` — an annotations-zone **capture-fidelity** attestation, a typed code with no prose and no lifecycle; record- or segment-scope (via `address:`). Two namespaces: `issue` (falls short, §4.3.3.2) and `sweep` (exhausted, §4.3.3.6). |
...
| **Container-member sidecar** | §7.2, §4.3.1.4 *(v45)*: a producer's per-member metadata file (an osxphotos `<member>.json`) paired by template to its frame — metadata **of** the frame, never a member: off the roster, no address of its own, read through the frame (`?path=<frame>&sidecar`), lifted onto the promoted frame's origin block. |
