...
**Timeline inheritance** *(3.11)*. A **timeline op** (`frame=`, `time=`, `time_range=`, `scenes=`, `transcribe`) addressed at a promoted media-stream leaf resolves through the leaf's containment lineage, against the container, with the leaf's own `stream_id=` composed in. This is the case §1.2 already states for chapter marks — *"tracks inherit the container's chapter marks at read time through lineage — never copied, because the track's own bytes do not carry them"* — applied to the timeline the marks sit on, and the reason is identical: **a bare elementary stream carries no container timing at all.** An Annex-B or ADTS byte-stream has no timescale and no duration; a decoder imputes a frame rate (from in-band parameter-set timing where present, from a default where not) and reports no duration to close a final span with. So a second of "leaf time" is not reliably a second of the timeline a stored address was computed in, and the stored address is the thing that must not move.

Three consequences, each of which the alternative gets wrong. Addresses on a leaf and on its siblings are in **one shared timeline**, so a video leaf's scene spans and its audio sibling's transcript spans can be read against each other — which is the whole point of §1.2's shared-timeline composition. Cut boundaries are measured where they will be resolved, so a `time_range=` resolves to the frames it names rather than to a neighbouring approximation. And the leaf stays a *leaf*: nothing is copied down onto it, so the container remains the single place the timeline is defined. `format=` is deliberately **not** a timeline op — an encoding change addresses no timeline, so a leaf's own playable rendering stays a leaf operation. As with every lineage chain, the container is blake3-pinned and immutable, and an absent one fails loudly at resolve rather than resolving against a substitute.
