...
That is not a theoretical gap. Measured on the §12.20 pilot deck: the container reports its video track at 60 fps and 616.53s over 36,991 frames, while the same track extracted to Annex-B probes as **25 fps with no duration at all** — the decoder's fallback, since the encoder wrote no VUI timing. An address resolved against the leaf's own timebase would land at 2.4× its intended position, and `frame=30` on that leaf produced an empty output rather than a wrong image only by luck of where the guard fired. A second of "leaf time" is not a second of the timeline a stored address was computed in, and the stored address is the thing that must not move.
