...
| `page=<N>&geometry` | PDF page | json | The page's text-layer line geometry: assembled visual lines (position, extent, text), chrome flags, heading candidates, and the non-chrome content band — exact coordinates for boundary-setting, read from the one place they are exact. |
