...
**What the sweep refused to do is the substantive part.** 10 records were held: 7 on one origin where a fixed crop was checked against its own transcribed body and did not contain it — the address was wrong beyond its grammar, so converting it would have laundered a wrong region into a legal one — 2 whose embed bytes will not decode at all (dims unknowable; a separate defect), and 1 genuinely ambiguous with no sibling to settle it. The two corpora also disagreed on the correct repair for the same overflow shape: on one origin the corners reading was proven by tiling and by crops matching bodies, while on the private statements the corners reading demonstrably **truncated** a transaction table mid-row and the size-with-overshoot reading was right. A blanket conversion would have silently destroyed content in one of the two. Ground truth is bytes, per record.

Residual, filed rather than papered over: 5 records address an `el=` past the end of their element list (`el=145` on a 9-element artifact — the same copy-lineage mechanism, and the resolve gate's other find), and a well-formed crop that names the wrong region is still beyond mechanical reach, which is why the authoring rule remains *resolve it and look at it*.
