...
The frontmatter (`---...---` at the top of the file) holds **only the bytes-identity header** — five fields *(3.1: `status` retired, §4.1; 3.5: `title`/`description`/`canonical` retired outright, below)*. Everything else lives in body blocks (§4.3) or surfaces as derived views (§9).
...
Both frontmatter hash fields (`transport`, `perceptual`) and the body-block hash fields (`<!--members--> transport`, `<!--segment--> perceptual`) use a uniform encoding:
...
| **Canonical hash** | ~~`canonical:`~~ — the canonicalized-content hash. *(Retired 3.5, §4.2.1: the write path was disabled 2026-06-28 after the strategy was found to collapse unrelated scanned PDFs, and the frontmatter field itself was removed, not merely disabled — see §7.1.)* |
