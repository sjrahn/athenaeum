...
| `context` | The umbrella for every **annotation** namespace — observations *about* a record. | Per-namespace overlays declared via the **context block** (`context/<namespace>/<id>`). One namespace: `issue`, capture-fidelity problems (§4.3.3). |
...
The annotations zone carries exactly one kind of observation: **where this record's representation of its bytes falls short.** One block family, the **context block**, and one namespace — `issue`. Context **never** contributes to the faithful content zone — it is a side-channel that accretes without disturbing the lossless body.
...
**Context is scarce by design, and scarce by construction.** Absent a real fidelity problem the annotations zone is **empty** — the normal state for the overwhelming majority of records. It is emphatically **not** a normalizer scratchpad: a normalizer MUST NOT emit commentary, summaries, running notes, or "what I did" prose as context, and it cannot do so by accident, because an issue carries **no prose field** (§4.3.3.2).
...
A promoted media-stream leaf needs no timeline redirect: its bytes are a single-track container of the source's own family (§1.2, §7.1 `framing`), carrying its own timescale and sample tables — so every timeline op (`frame=`, `time=`, `time_range=`, `scenes=`) resolves on the leaf directly, in the same timeline its container reports. An audio leaf's `transcribe` likewise runs on the leaf's own pinned bytes.
...
- `framing` — **the stamp that admits an engine into the identity path.** A promoted stream leaf is muxed, not reframed, so the producer is a versioned tool rather than pure byte-work. The tool, its version, and a **sample count** are attested onto the leaf's artifact block — the same shape and the same purpose as `addressing:`'s `parser` + `elements`:
...
Returns structured records — each issue carries its id/subtype + universal fields + optional `address:` + any id-specific fields. (`issue` is the only namespace, so this view and the context view coincide.)
...
- **Deterministic stream extraction.** Track promotion from media containers (§1.2) requires member-byte determinism. The discipline: pin the invocation and attest the producer — `-c copy -fflags +bitexact`, a `framing:` stamp carrying muxer, version and sample count (§7.1), an independent sample-table oracle that does not share the producer's implementation, and a suite canary against a fixed hash. Cross-version stability is the flag's designed contract and is checked by the canary rather than assumed. Content-addressing plus continuity-gated supersession (§12.8) remains the net if a pinned extraction ever shifts bytes.
...
| **`framing:` stamp** | A promoted media-stream leaf's attestation of the muxer, its version, and a sample count (§7.1) — the self-check that admits a muxing engine into the identity path on measurement rather than on faith. |
