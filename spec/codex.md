...
- **`roots`** — include the listed concepts and their subgraph, traversed **in both directions** (claims' `object` targets; **structured-value entity references** — `{"entity": <id>}` elements inside a claim's `value`, the roster shape of `facts/SCHEMA.md` conv. 7 that stores an event's attendance and a group's members, treated exactly like a claim `object`; facts whose claims target an in-scope fact — relations store on the spoke, so a hub's subgraph is mostly incoming; edges whose `subject`/`participants` intersect the scope, and their members), transitively, **within the traversal type filter**: `traverse_types` when declared, else `types`. The bound is what keeps a hub-adjacent private graph out — one hop through an unlisted type never happens.
