...
**Version 21** *(2026-08-17, owner ruling)* is the **custody amendment** — Part II lands the store mechanics v18 waited on: declared artifact **locations** (content-addressed store roots, local or remote, plus attached in-place trees indexed by an attest pass — `spec/corpus.md` §12.1.1, §12.9.2). For this part: a dataset's **adapter** may derive schema-first from its mirror-artifact record's mime overlay (`ref_adapter`, `spec/corpus.md` §7.1), the manifest declaration remaining as override; the v18 `path:` override **deprecates** in favor of an attached location over the mirrors directory (§6.5's resolution unchanged in shape — the artifact blake3 remains the pin); adapter sidecar indexes are cache-mold derived state under `cache/refidx/`, gc-exempt (§6.5). Consequentially, mirror-artifact **records become real** for every registered dataset — closing the standing §6.4 gap where a record-less mirror's `ref://` sensitivity fail-closed **private**: with mirrors ingested through public-tenancy origin overlays (kiwix.org, geofabrik.de), citations into public reference datasets become publishable evidence, which is what the tenancy derivation always specified and custody never delivered.
...
**Version 17** *(2026-08-15, owner ruling)* activates the reference-dataset layer §6.5 had deferred. Datasets register **multiple snapshots** — a tag-keyed map with an explicit `latest:` default — and the citation grammar gains an optional pin: `ref://{dataset}@{tag}/{id}` (bare cites track `latest` and are drift-flagged; pins freeze, deliberately). **Mirror bytes become corpus artifacts**: each snapshot's mirror is a terminal-contract record addressed by blake3 — provenance, integrity, distribution, and gc through the corpus's own machinery — while the entry grain stays un-corpus-like and `ref://` remains the sole citation surface for mirror content; a mirror hash cited directly as claim evidence warns (§13.1). The snapshot binding stamps tag **and** artifact hash (§13.2 — the hash is the true pin, since the manifest is untracked deployment state). Blanket-public `ref://` sensitivity retires for tenancy derivation through the mirror record (§6.4); the §5.4 bar counts a dataset as one independent source; harvest never sweeps mirror content and coverage discharges at the mirror grain via the dataset's own concept (§6.5). Anchor grammar for `ref://` stays out of scope (Part I §9). Nothing cited `ref://` before this version — the activation has no migration.
...
**Version 1.9** re-keys evidence sensitivity from corpus membership to **record tenancy** (§6.4), preparing the single-corpus consolidation (owner ruling, 2026-08-14): a record's tenancy derives from its origins — an origin overlay MAY declare `tenancy: public | private`, a promoted member inherits through its origin's `corpus://` lineage, and a record declaring nothing falls closed to its holding corpus's manifest `visibility:`. Any-public-wins across origins and holders, preserving 1.0's same-bytes rule exactly ("bytes present in a public corpus are public evidence even if also captured privately"). With every corpus-visibility pairing mirrored by declarations, the derivation is invariant across the physical merge of the corpora — which is the acceptance gate the consolidation runs behind. The corpus itself never consumes `tenancy:` — a corpus asserts nothing about privacy; the declaration is deployment metadata the ledger reads, exactly as harvest reads origin facts (§10).

**Version 1.8** grades deferred surfaces instead of gating them (the deferral principle — ruled by the owner, 2026-08-14): a record whose declared citation surface has not yet been persisted (a `segments`-surface mime with no persisted segments, corpus §7.1) is **citable at reduced strength**, never a verification error. Its claim evidence is **deferred** — admissible on any rung, excluded from the §5.4 authentication bar (a claim carried only by deferred evidence cannot be `confirmed`), and reported by verification as its own class beside the honestly-unverifiable residue (§13.2.4). **The citation itself is the normalize demand**: validation aggregates deferred citations per record, replacing 1.4's enqueue-before-cite discipline with cite-then-pressure; the 1.4 interpretation warning (a reference without a typed need) retires with the discipline it policed. What verification can still check mechanically on such a record, it checks — attested byte-facts and derivation-op output remain verifiable surfaces (§6.3) — but the bar keys to the record's surface state, not the entry's luck: a formed span attests what the artifact *presents*; a raw DOM proves only that the bytes *contain* the text. Deferral resolves by forming — when the declared surface lands, the same evidence starts counting and its held quotes verify on the next pass.
...
> at least one **authoritative** evidence artifact, **or** two or more **independent** sources — records of different provenance (different corpus hashes) and/or `ref://` citations, one reference dataset counting **once** however many of its entries are cited *(17)* — and nothing else in the graph contradicting it.
...
- **Resolution is downward.** The resolver maps `(dataset, tag)` → the snapshot's mirror-artifact blake3 → bytes through the corpus store — *(18)* a snapshot declaring the interim `path:` override (`spec/athenaeum.md` §2.3) is read in place from that file first, the artifact hash remaining its identity; *(21)* the override is deprecated: an **attached location** over the mirrors directory (`spec/corpus.md` §12.1.1) resolves the same in-place file through the store proper, and an adapter needing a sidecar index (an OSM extract's) keeps it as cache-mold derived state under the corpus's `cache/refidx/`, keyed by mirror blake3, gc-exempt by name (`spec/corpus.md` §12.8) — and the dataset's adapter renders the entry (an article's text, a release's fields) for quote verification (§13.2). Content resolution requires the mirror bytes locally materialized; where they are absent — or the dataset's adapter is unavailable in the verifying environment — content verification reports **unverifiable**, never failure, while grammar, registration, and snapshot-binding checks stay mechanical everywhere. Claim evidence citing a mirror's corpus hash *directly* draws a warning: the record exists for provenance and distribution; its content's honest citation surface is `ref://` (§13.1).
...
- **Pin the tuple when freezing.** A consumer freezing a deliverable SHOULD record its reproducibility tuple — the ledger commit, the touch identity of every record cited or resolved (§13.2), and every `ref://` citation's stamped snapshot binding — tag + mirror-artifact blake3 (§6.5, §13.2) — so ledger or corpus movement beneath a frozen build is a *detected transition* (`ath ledger worklist` names the dependents), never silent rot discovered by readers.
