...
- The derived citation MUST resolve: **`corpus://{hash}`** with the full 64-hex blake3 — anchor parameters per the corpus functional-URI grammar (`spec/corpus.md` §6): **address-class ops** (`el=`, `page=`, `time_range=`, `frame=`, `page=N&bbox=`, `path=`, `msg=N&part=M`, `prop=`, `turn=`, …), optionally ending in **one reading-class op** (`page=N&text`, `part=M&text`, `members`), or a `#fragment` — never a view, instrument, or engine op (corpus §6.2 **op classes**, v43: a contact sheet, a `mark=` overlay, a `probe`, a page's `geometry`, a live `transcribe` are tools for the normalizer or the querier, not evidence; an anchor carrying one is a citation defect, §13.2) — or **`ref://{dataset}/{id}`** into a registered reference dataset (§6.5). (Rosters and `based_on` references, which carry direct URIs, obey the same grammar.)
...
- **Anchor only as precisely as verified.** A record-level cite is always safe; a wrong anchor is bad provenance — worse than none. Segment addresses printed by the corpus tooling (`corpus body` / `corpus toc`) are ground truth. Every anchor is expected to resolve mechanically — against the record's stored rendering or through a derivation op (`spec/corpus.md` §6.2) — and §13.2 verifies it there; an anchor that cannot be mechanically resolved is a citation defect, not a tolerated form. **An anchor is never prose** *(v47)*: it is functional-URI parameters every one of which names an op or address axis the corpus knows, or a `#fragment` — words describing the place ("the transactions table, page 2") belong in `note`, and an anchor made of them is a check error (§13.1), never a record-wide cite in disguise. (An anchor whose surface has no text projection — an image region, an untranscribed span — still resolves and bounds-checks; it simply carries no `quote`.)
- **Quotes are verbatim spans** of the resolved content at the cited anchor — they exist to be machine-checked (§13.2). Paraphrase belongs in `note` or `reasoning`, never in `quote`. A `|` in a quote separates table **cells**, and cells cite **one row** *(v47)*: a transaction is quoted from its own row (`Mar 04 | SAFEWAY #4907 | 52.18`); a claim resting on two rows cites each with its own entry, and what a row's cells cannot carry — which column a value sits in — belongs in `note`.
...
```jsonc
{
  ...
  "proposes_new": { "id": "…", "type": "…", "name": "…",            // hypotheses only (v47): a fact
                    "claims": [ /* draft Claim objects on it */ ] },  //   not yet minted, and its claims
...
**`proposes_new`** *(v47)* is the same edge for a thing the ledger has **no fact for yet** — the hypothesis that an event the trip was for, or a venue the gig was held at, exists at all. It names the concept to mint (`id` — a slug in the shared namespace, §4.1, **untaken** while the hypothesis is open; `type`; `name`) and the draft claims it would carry (`claims`, each a draft Claim whose id targets the new fact, citing inline like `proposes`). A `proposes` beside it may target the new fact or name it as its `object`. Minting a stub ahead of the doubt is the wrong shape — a stub asserts the thing exists (§4.2), which is exactly what the hypothesis has not established — and a mint proposal left in prose (`statement`, `would_resolve`) cannot promote. Promotion mints `facts/{type}/{id}.json` and lands every draft, all validated before anything is written: a promotion lands whole or not at all.

### 7.3 Lifecycle

```
             ┌── promoted ──→ the content now lives as claim(s) — on a fact `proposes_new` minted, where
             │                one is proposed; resolution names the `proposes` claim, else the minted fact
hypothesis ──┤   (open)
...
**Epistemics** — the authentication bar for every `confirmed` claim, counting only verifiable-surface evidence (§5.4) — per element for array-valued claims, naming each element that fails; `element` bindings well-formed: integer, in range, on an array `value` only; `disputed` ⇄ standing `correction` pairing, with `challenges` pins current (a pinned claim edited since its challenge was filed flags the correction for re-review, §7.3); `reported` claims carrying `attribution`; retired vocabulary unused; `proposes`, `proposes_new`, and `challenges` objects well-formed (against §5.1, §7.2, and §7.3) — a draft claim's target and `object` name a known fact or the one its hypothesis's `proposes_new` mints, and an open hypothesis's `proposes_new.id` is untaken; presence claims well-formed: `presence` never co-occurring with `value` or `object`, `presence` from `none | some`, no `element` bindings on presence claims (§5.5).

**Evidence** — URI grammar and resolution discipline (§6.2), every anchor param naming a known op or address axis (a prose anchor is an error, v47); cited and rostered records exist (bare-hash resolution in the corpus); `ref://` citations name registered datasets — and, when pinned, registered snapshot tags: a pin whose tag is no longer registered is an error, a dangling pin — (§6.5); claim evidence citing a corpus hash registered as a mirror artifact warns — the content's citation surface is `ref://` (§6.5); roster representation fields (§4.2) are well-formed — `modality`/`derivation` terms registered (§8), a `derived_from` URI rostered on the same concept; every citation resolves to a **verifiable surface** and passes §13.2.
...
1. **Anchor resolution**: every span parameter resolves against the cited record (the segment address exists; the page/region/time-range is within bounds). **The anchor is the quote's scope** *(v47)*: an axis anchor (`page=2`, `msg=3`, `time_range=…`) scopes to the stored segments within that unit; a **region** anchor — one carrying `bbox=`, a place finer than its unit — scopes to the stored segment(s) at exactly that address, and a page region (`page=N&bbox=…`) that is no stored address scopes to the stored segments of its page its region overlaps (overlapping none, it does not resolve) — never to the whole page, where a quote could assemble itself from unrelated segments.
2. **Quote verification**: every `quote` is found verbatim (modulo whitespace and presentational-markup normalization — inline markers such as `<u>…</u>` vanish before matching, so a quote cites the *rendered* text and never truncates around markup) within the content the URI resolves to — and, where that content holds a table, a quote carrying `|` cells within **one table row** of it *(v47; a markdown pipe-table row or an HTML `<tr>` of the stored rendering — over a scope with no table, `|` stays a plain in-order separator)*: in-order matching across the whole scope let a quote join one row's merchant to the next row's amount. A quote that fails against a **re-derived** surface (a normalization or derivation-op pass that shifted the text) is flagged for **re-anchoring by re-evaluation within its anchor's resolved scope** — the anchor is the repair scope; record-wide fuzzy matching is never the repair. The citable content is the record's **faithful rendering** — segment bodies under their form (form-span renderings) and the derivation ops' mechanical output — plus attested byte-facts (artifact-block fields, the members roster, structural byte-marks; §6.3). *(Residue clause)* A record ingested before the corpus's faithfulness sweep may still carry retired descriptive fields; verification reads them **tolerantly** where present — an existing anchor into one still resolves and its quote still checks — but a fresh claim's evidence may never cite one: new evidence cites the faithful rendering or the byte-facts, never a field the corpus grammar no longer writes.
3. **Snapshot binding**: verification records the cited record's content state (its latest `touch` identity) — for `ref://` evidence, the **resolved snapshot's tag and its mirror-artifact blake3** (§6.5): a bare citation resolves through `latest` and is flagged for re-verification when the resolved tag or its artifact hash moves (the hash is the true pin — the config is mutable, the hash is not; a re-pointed tag flags exactly as a moved `latest` does); a pinned citation is drift-free by construction, its residual failure modes being mirror absence (honestly unverifiable) and tag deregistration (a §13.1 error) — on the fact's **sources entry**: one binding per *(fact, source)*, shared by every evidence entry anchored to it, so a later authoring pass or mirror update flags the evidence for re-verification instead of silently rotting. Re-stamping is touch-keyed: an unchanged touch never rewrites the binding. For evidence whose anchored content resolves through a **derivation op** rather than the stored record body, the binding additionally pins the op's version label (`spec/corpus.md` §6.4): the touch chain does not move when resolver tooling upgrades, so the op pin is what flags a derived surface's drift — exactly as the touch flags a re-authored one. The binding's `verified` object carries an **`ops` map** — `"ops": {"<axis-param>": "<engine-pin>"}`, e.g. `{"prop": "vcard-prop@1", "path": "archive-path@2"}` — one entry per derivation-op axis any of that source's evidence resolves through, valued with the engine pin current at stamp time (read from the resolver's own registry introspection, never hand-written). Anchors resolvable from the record's stored body pin nothing — the touch already covers them; only derived-surface resolution pins. At verification, a source whose evidence resolves through an op whose current engine differs from the pinned one is flagged for re-verification (warning), and re-stamping is pin-keyed exactly as it is touch-keyed: an unchanged pin never rewrites the binding. Correspondingly, verification MUST resolve a derived-surface anchor **through the corpus resolver** (the derivation ops themselves — ledger depends on corpus, so this is a library call, never a re-implementation) and check the quote against the derived output; the honestly-unverifiable class narrows to anchors whose op the verifying environment genuinely cannot run (artifact bytes absent, an optional extra not installed), which stay honestly unverifiable, never errors. **Class before resolution** *(v43)*: an anchor is checked against the corpus op classes (corpus §6.2) before any resolver call — a **view**, **instrument**, or **engine** op in an anchor is a citation defect at the claim's severity (an error on a confirmed claim), reported as such and never resolved, never held as unverifiable: the honest-unverifiable path is for surfaces this environment cannot run, not for anchors that should not exist. Only address-class ops, optionally ending in one reading-class op, reach the resolver.

Verifying against derived surfaces makes member anchors first-class: a `?path=` member's bytes derive mechanically through their container, so anchors and quotes into members verify like any other surface — no honest citation form remains unverifiable by construction. A container-member sidecar (Part II §7.2) is not a member and has no address of its own *(v45)*: it is cited **through its frame** — `?path=<frame>&sidecar`, or `?sidecar` on the promoted frame — a reading an anchor may end in, whose quote verifies against the sidecar's verbatim bytes.
...
// facts/continuity/bsg-reimagined.json (excerpt)
{
  ...
  "ontology": {
    ...
    "types": {
      "vessel": { "extends": "cco:Material Artifact", "description": "An in-universe craft." }
    }
  },
