...
- **Body-empty marker** — every other segment, in three cases. (a) The three non-text atoms (`image`, `audio`, `video`) are positioning markers in document flow that link to a matching embed (by address membership) for the asset's metadata and description. (b) A `text/<id>` overlay whose declaring schema sets `enables_lossless: false` marks a *typed but non-lossless* text region — e.g. a live, formula-driven table whose displayed values are a single-execution snapshot rather than faithful content; its meaning goes in the segment's `description:`. (c) A bare `text` member of a **container** the drafter renders as a manifest rather than transforming — e.g. a text file inside a kept-whole archive. Its bytes are verbatim and content-addressed, resolvable through its address (the functional URI), and the drafter performs no transformation (no OCR / layout / markup cleanup) that a body would capture; copying the text into the body would only duplicate the immutable artifact. So it is positioned but not transcribed — its bytes resolve on demand and its description rides on the matching embed, exactly like case (a). In all three the segment body is empty.
...
- **`text`** — the atom whose segments carry segment bodies, with two exceptions. Plain prose by default; shaped by a `text/<overlay>` for structured lossless forms. (1) A `text/<overlay>` declaring `enables_lossless: false` is a body-empty marker like the non-text atoms, with its meaning on the segment `description:` (see the body permission rule). (2) A bare `text` member of a container the drafter renders as a manifest (not transforming it) is a body-empty marker too — verbatim, resolvable bytes that a body would only duplicate, so they and the description ride on the embed (§4.3.2.2 case c).
