...
- **Body-empty marker** — every other segment, in three cases. (a) The three non-text atoms (`image`, `audio`, `video`) are positioning markers in document flow that link to a matching embed (by address membership) for the asset's metadata and description. (b) A `text/<id>` overlay whose declaring schema sets `enables_lossless: false` marks a *typed but non-lossless* text region — e.g. a live, formula-driven table whose displayed values are a single-execution snapshot rather than faithful content; its meaning goes in the segment's `description:`. (c) A bare `text` member of a container whose faithful bytes are **deferred to its matching embed** rather than inlined — a large embedded text file positioned in the manifest but not transcribed into the body; like case (a), its bytes resolve through the embed's address (the functional URI) and its description rides on that embed. In all three the segment body is empty.
...
- **`text`** — the atom whose segments carry segment bodies, with two exceptions. Plain prose by default; shaped by a `text/<overlay>` for structured lossless forms. (1) A `text/<overlay>` declaring `enables_lossless: false` is a body-empty marker like the non-text atoms, with its meaning on the segment `description:` (see the body permission rule). (2) A bare `text` member of a container whose faithful bytes are deferred to a matching embed (not inlined) is a body-empty marker too — its bytes and description ride on the embed (§4.3.2.2 case c).
