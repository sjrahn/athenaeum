...
The annotations zone is a single block family — **`context`** (spec §4.3.3) — drawing overlays from a reserved top-level `context/` umbrella, one namespace per kind of observation. The bundled namespaces are `issue` (problems, with the severity/resolution/detector shape) and `reference` (cited sources, the §4.4.5 ladder); a corpus may add its own. The block stores as `{namespace, id, subtype, fields}` — the same shape as a classify block — in `post.metadata["_contexts"]`; emitted `<!--context <ns>/<id>-->`. Context never feeds the canonical hash or the faithful body segment.
