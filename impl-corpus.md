...
### 2.13 Dependent references (`capture.references`)

A page's most relevant outbound links are part of the capture itself — a product-detail page's product manual / spec sheet far more than the other hundred links on the page. The per-host `capture.references` overlay section (spec §7.2) declares which links those are; the tooling emits a `reference` context block (§4.3.3.3) for each and, opt-in, fetches it depth-1 as its own record. The home is one module, `corpus.references` — *declare, match, emit* — with the *fetch* delegating to the existing capture/crawl machinery. Opt-in throughout: no rules → entirely inert.

- **The rules** (`references.parse_rules` → `ReferenceRule`). A `capture.references` list of `{match, role, capture, cross_host}`. A rule's `match` keys (`selector` CSS, `href_pattern`/`text_pattern` regex via `re.search` on the resolved href / anchor text, `rel` token) are **ANDed**; rules are **ORed**. `role` (corpus-local label — `manual`, `spec-sheet`) rides onto the emitted reference; `capture: true` opts the target into the depth-1 grab (default false = annotate only); `cross_host: allow` (the default — manuals are off-host) lets a match reach another host, `same` host-restricts it. Parse-tolerant: a non-mapping entry, a rule with no match key, or a bad `cross_host` is skipped, never fatal.
- **Matching** (`references.match` / `matches_for_record`). `match(html, base_url, rules, *, primary_host)` parses the DOM (BeautifulSoup), scopes candidate anchors by `selector` (else all `<a href>`), applies the AND filters, resolves relatives against `base_url`, normalizes (`urls.normalize`), drops non-crawlable hrefs (`urls.is_crawlable_href` — the same filter `links`/`crawl` use), enforces `cross_host: same`, and **dedupes by URL** (DOM order, first rule wins for role/capture). `matches_for_record(corpus_root, post, html)` is the record-aware entry: loads the host's rules, applies them against the record's primary origin URI, and excludes self-links (the record's own origin URIs). Shared by draft emission, `links --references`, and the grab.
- **Emission is draft-stage** (`references.emit_overlay_references`, called from `draft.derive_record` right after the per-host canonical content-scoping — both are draft-time origin-overlay reads). HTML-only and **record-scoped in v1** (no segment anchor: the "Manuals" link usually sits in un-segmented chrome, so a clean record-scope beats a brittle DOM→segment map — see open items). Each match emits one `<!--context reference-->` with `provenance: auto`, the rule's `role`, tier-1 `attribution_text` (link text), and tier-2 `source_url` (resolved href). It **stops at tier 2**: the mechanical drafter writes **no** tier-3 `source_uri` and reads **no** corpus state (`emit_overlay_references` does no `find_by_uri`), keeping `draft` a pure function of the artifact (spec §4.3.3.3 — same artifact in, same bytes out, regardless of what else the corpus holds). Whether `source_url` is itself a record is a **read-time derived edge**: `derived_views.references` (§9.9) resolves it against the URI index and surfaces `captured`/`resolved_uri` (mirroring `links --references`), so the edge self-heals (`captured ⇄ pending`) under capture/removal/supersession instead of dangling as a stored pointer. Idempotent for free: `draft` runs only on a clean stub and `redraft` re-stubs first (clearing context blocks), so emission only ever appends.
- **The depth-1 grab** (`references.fetch_references`). `select_for_capture(matches, force=)` picks targets — `force=True` (`--with-references`) all, `force=False` (`--no-references`) none, `force=None` (default) the rules' own `capture: true`. Each selected, not-already-captured (`find_by_uri` dedup) target is fetched once via `capture_and_ingest` — depth is fixed at 1 (a grabbed target is ingested as a stub, never expanded; multi-hop stays `crawl`'s job, spec §11). Best-effort: a per-target failure is recorded, not raised. **Surfaces:** `corpus capture --with-references` / `--no-references` (the inline grab, in `capture._maybe_grab_references` after the primary ingest; honors per-rule `capture:` by default); `corpus crawl --references [seed]` (the deferred sweep — `force=True` over the seed's record, or every record when no seed; `--dry-run` lists pending targets). Robots/politeness for the grab rides on whatever `capture_and_ingest` enforces (it does not gate on robots today — a follow-up if a non-overlay-declared grab is ever added; the overlay author has explicitly opted these targets in).
- **Discovery + view.** `corpus links --references <record>` previews the declared subset (the typed projection of `links`), each line annotated `[role=…, captured|pending, auto?]`. The `references` derived view (`derived_views.references`, spec §9.9) is the `reference`-namespace projection (sibling of `issues`/`concepts`), surfaced in the API `record_detail` as `references`. The reverse edge (which records reference *this* one) is a corpus-wide read, not indexed (spec §9.9 / §11).
...
- **`text/html`, `application/xhtml+xml`** → the **mechanical** HTML drafter (`draft/html.py`): it removes only non-rendered infrastructure (scripts/styles/comments), assigns `el=N` addressing to every content element of the raw artifact (the shared `transforms.html.is_addressable` predicate — content-bearing blocks and inline-media carriers, not layout-only `div`/`span` wrappers), emits dedup'd embeds, and emits one cleaned-`<body>` text segment. The `el=N` axis is the structural `_ADDRESSABLE_TAGS` set (content blocks + `<img>`) **plus inline-media carriers**: `<video>`/`<audio>` (materialized from their inline `<source data:…>`) and `<a href="data:…">` attachments (vCards, files). Every carrier becomes a dedup'd **embed** (`transport` = blake3 of the decoded bytes; `media_type` from the data URI; an attachment keeps its recovered `filename`, an image its `width`/`height`/`alt`), and the carrier's base64 `data:` payload is **stripped from the body** — the carrier survives as a body-empty `el=N` positioning marker (critical: a single inline video can be hundreds of MB). A bare `<a>`/`<source>` is **not** addressable — only a `data:`-bearing one — so per-message `<a href="sms://…">` deep links never consume an index. The carrier→bytes round-trip lives on the resolver side (`transforms/html.py`): `el=N` selects an `HtmlElRef`; a terminal `<img>` ref renders to a PIL image (cache PNG, and `el=N&bbox=…` promotes it first), a media/attachment ref materializes to raw bytes (cache the media's native extension, sidecar its real MIME). `draft/html.py` imports the predicate + carrier helpers from `transforms/html.py`, so drafter and resolver name the same elements `el=N` by construction (`test_drafters.py` asserts the lockstep). It does **not** strip page chrome — a universal tool can't reliably tell chrome from content (e.g. ASP.NET WebForms wraps the whole page in one `<form>`), and a wrong guess drops content silently. Chrome removal (nav/ads/cookie notices) is therefore a **capture-time, per-host** decision: list the selectors to delete in the origin overlay's `capture.interactions[].remove` (§2.8). Structural recovery (headings/tables/lists/equations) is the normalizer's job.
- **`application/pdf`** → `draft/pdf.py` is **uniform and mechanical**: every PDF drafts to the same shape — one **body-empty `image` segment per page** addressed `page=<N>`, **sectionless** — plus the `/Info` frontmatter fields (`page_count`, `title`, `author`, `producer`, `creation_date`, `modification_date`). The drafter makes **no** born-digital-vs-scanned determination and extracts **no** text; the page raster is the faithful transport unit. Everything an agent needs to determine a page's shape and recover its content is exposed **at normalize time through the resolver** (`corpus.pdf_introspect`, surfaced as functional-URI ops, §6.2): `page=<N>&text` (embedded text layer via pypdf, surrogate-scrubbed), `page=<N>&words` (per-word boxes in `[0,1]` page fractions, origin top-left, via pypdfium2's text page), `page=<N>&probe` / `probe` (per-page + whole-doc structural signals — dimensions, rotation, text-char count, `image_coverage` via the content-stream CTM walk over image `Do` ops, an invisible-text `Tr 3/7` flag, and an advisory `shape_hint` of `born-digital`/`scanned-ocr`/`scanned-no-text`/`no-text`), and `outline` (the TOC bookmark tree). The normalizer uses these to: pull born-digital text into prose segments; OCR a scan into `text/ocr` at `page=<N>&bbox=...` (so the corpus owns OCR provenance — engine/confidence/region — rather than laundering a pre-baked machine-OCR layer); and wrap pages into `<!--section-->` blocks from the outline (`pages=<start>-<end>`). Resolver mechanics: the PDF working kind is `pdf`; `page=<N>` yields an intermediate **`pdfpage`** selector so a sub-op reads the page directly rather than OCR'ing a render. A **terminal `pdfpage`** — or an image op / `bbox=` after it — auto-renders to image (`resolver._resolve_handler` + `_render_pdfpage`), so a bare `page=<N>` and `page=<N>&bbox=...` resolve to the page image exactly as before, and a `page=` image segment stays a self-slice needing no embed (`lint._self_slice`). The text/probe ops read the source from disk via pypdf (`ctx["artifact_path"]`); `words`/`probe`/`outline` cache as `json` (the cache sidecar is `<name>.json` so it can't collide). Canonical: `blake3-canonical-pdf` (per-page extracted text — byte-derived, independent of draft shape; persist currently disabled, §7.1 note).
...
**Context is scarce by design (spec §4.3.3).** The mechanical namespaces (`issue`, and the deferred `aside`/`relation`) auto-populate only on real signal and only where schema-gated — `aside` exists *solely* where an overlay declares the extraction. `reference` is **dual-source**: an interpretive citation the normalizer finds in the content, *or* a mechanical, overlay-declared dependent reference emitted at draft (`provenance: auto`, §2.13) — both require a referent actually present (a citation, or a declared link in the page). The normalizer is contractually forbidden from using context as a scratchpad (commentary / summaries / running notes); absent real signal the annotations zone is empty. There is deliberately **no** bundled free-text `note` namespace — that would invite exactly that flooding.
...
- **Reference.** Dual-source. **Interpretive** (normalizer-/human-asserted): the durable home for a cross-reference §3.2 couldn't resolve mechanically — a casual mention gets a `reference` context anchored by `address:` + a verbatim `quote:` and researched up `attribution_text → source_url → source_uri`. **Mechanical** (overlay-declared, `provenance: auto`, with a `role` field): the dependent-reference emission of §2.13 — tier-2 (`source_url`) only; the intra-corpus edge is resolved at read time, never stored. Lint `reference-unresolved` flags a stored `source_uri` that doesn't resolve to a captured record — now only ever an *asserted* one, since the mechanical drafter writes no `source_uri` (§2.13); `context-namespace-unknown` flags a block whose namespace has no `context/<ns>` overlay.
...
- **Decompose/compile.** The manifest keeps the dedicated `issue <id> sev= res= detector=` line for the issue namespace and adds a generic `context <ns>/<id> k=v…` line for the others (`recordbuild.add_context`). Two emit conventions keep the working dir hand-editable: **`status` is authored only on the manifest `record … status=` line** (the advertised, editable place — `read_workdir` applies it; it is *not* duplicated in `meta.yaml`, where an edit would be a silent no-op); and **`meta.yaml` renders a multi-line string as a YAML block literal (`|`)** (`recordbuild._MetaDumper`) rather than a single-quoted scalar, so a multi-line `description` never reads as a truncated stump. An **address list** in the manifest is bracketed and **`|`-separated** (`[a|b|…]`), not comma-separated — a single address (e.g. `bbox=x,y,w,h`) already contains commas (`recordbuild._fmt_addr`).
...
- **`corpus overlay <namespace>/<id>`** — the field-spec table (types, `semantic_type`, required) for a candidate classification, so the normalizer fills a classify block without reading YAML. Given a bare **host** instead, it falls back to the **origin overlay** (`schema/origin/<host>.yaml`) and prints its host match, declared operational sections, and `normalization.guidance` — so a host's origin guidance is inspectable at normalize time (the same guidance `corpus guidance` shows once that overlay is applied), not only its composite overlays.
- **`corpus lint <target> [--json]`** — the conformance gate; `--json` emits a **single JSON array** of findings (each = the `Finding` fields + `record_id`), across all records when `<target>` is omitted, so a consumer `json.load`s the whole stream (not line-delimited).
...
**Operator runbook (`corpus workflow`).** The two operating modes and this result lifecycle are surfaced to users centrally via `corpus workflow normalize-loop` — guidance for *running* the tooling, distinct from `corpus guidance <id>` (per-record normalization guidance). Runbooks are markdown under `corpus/workflows/` (`corpus.workflows` loads them via `importlib.resources`, like `schemas_default`; `_cli/workflow.py` lists them, shows a runbook's overview, or narrows to a `## ` section: `corpus workflow normalize-loop standing`). The queue verbs' `--help` cross-reference it (`_common.attach_workflow_note`). Keeping the runbook in the package means the operating modes are maintained **once, in the tooling, out of band of the corpora repos** — not duplicated per corpus, consistent with the tooling staying agnostic to any particular corpus.
