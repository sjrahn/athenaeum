...
- **`draft.strategy: zip-manifest`** → the general `self_contained`-zip drafter (`draft/zip_manifest.py`), selected by *strategy* not schema id (`corpus.draft.STRATEGY_REGISTRY`), so one drafter serves any number of bundle types (a diagnostics export, a config/log backup) that differ only by the schema's `draft.manifest` config. It renders the archive's file tree as a pseudo-ToC: members are addressed `path=<relpath>` (root-stripped per `manifest.root_strip`); `manifest.sectioning: top_level_folders` groups each top-level folder into a `<!--section-->` (`path=<folder>/`, root-level members into a synthetic `(root)` section), else `flat` lists them as top-level segments. A small text member matching a `manifest.inline` glob (≤ `max_inline_bytes`) becomes a `text` segment carrying its decoded body (faithful + searchable); **every other member becomes an `<!--embed-->`** (`transport` = blake3 of the member bytes, `media_type` = detected, addressed `path=<relpath>`) the normalizer later describes — with a body-empty positioning marker of the member's atom (text/image/audio/video; spec §4.3.2.2 case c for deferred text) when it has one, or embed-only for an opaque binary (the EPUB orphan-image pattern). Every member's bytes materialize through `transforms/zip.py`: `corpus://<id>?path=<relpath>` → the member bytes (`working_kind: zip` + the `path=`→`bytes` transform; `corpus.ziparchive.resolve_member` re-derives the wrapper root so a root-stripped address round-trips). The artifact block carries `member_count` / `uncompressed_bytes` and, when there's a single wrapper dir, its name as the `title` candidate. An empty archive yields a blocking `partial-content` issue. This is the drafting analogue of overlay-driven capture: the *type* is recognized and the *representation* tuned entirely from the corpus's mime-schema overlay, with no bundle-specific code in the shared package.
