...
- **`draft.strategy: zip-manifest`** → the general `self_contained`-zip drafter (`draft/zip_manifest.py`), selected by *strategy* not schema id (`corpus.draft.STRATEGY_REGISTRY`), so one drafter serves any number of bundle types (a diagnostics export, a config/log backup) that differ only by the schema's `draft.manifest` config. It renders the archive as a **description manifest, inlining nothing** — the key distinction from the PDF/HTML/EPUB drafters: those *transform* their source (rasterize, OCR, clean markup) and so must materialize a body, whereas a zip member is already verbatim, content-addressed bytes that resolve on demand, so copying the text into the body would only duplicate the immutable artifact. **Every member becomes an `<!--embed-->`** (`transport` = blake3 of the member bytes, `media_type` content-sniffed, addressed `path=<relpath>`, root-stripped per `manifest.root_strip`) the normalizer later describes; a member with a renderable atom is additionally positioned by a **body-empty marker** segment at the same address (text via `_is_text` content-sniff — UTF-8, no NULs — so a `.cfg`/`.conf`/extensionless log is correctly text, not extension-misclassified binary; spec §4.3.2.2 case (a) for media, case (c) for a verbatim text member), while an opaque binary is embed-only (the EPUB orphan-image pattern → a soft `embed-unreferenced` warning). `manifest.sectioning: top_level_folders` groups each top-level folder into a `<!--section-->` (`path=<folder>/`, root-level members into a synthetic `(root)` section), else `flat` lists the markers as top-level segments. Every member's bytes materialize through `transforms/zip.py`: `corpus://<id>?path=<relpath>` → the member bytes (`working_kind: zip` + the `path=`→`bytes` transform; `corpus.ziparchive.resolve_member` re-derives the wrapper root so a root-stripped address round-trips). The artifact block carries `member_count` / `uncompressed_bytes` and, when there's a single wrapper dir, its name as the `title` candidate. An empty archive yields a blocking `partial-content` issue. This is the drafting analogue of overlay-driven capture: the *type* is recognized and the *representation* tuned entirely from the corpus's mime-schema overlay, with no bundle-specific code in the shared package.
